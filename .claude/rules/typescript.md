...
3. bun run oxlint のwarning, error を解消して、oxlint の warning, error は常に無い状態にして
...
63. コードの変更をしたら`bun run tsc`, `bun run oxlint`, `bun run test` が常に正常終了することを確認してください。
64. コードを変更するたびに`bun run tsc`, `bun run oxlint`は実行をして。正常終了しなかった場合はすぐに修正をしてください。
...
70. テストコードの変更をしたら`bun run tsc`, `bun run oxlint`, `bun run test` が常に正常終了することを確認してください。
