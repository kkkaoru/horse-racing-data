...
bash research/chronos2/run-local.sh "$PY/pytest" -p no:cacheprovider --basetemp="research/jra-20260913/test-$RUN_ID" --cov=viewer_mcp --cov=race_rank_review --cov=dedicated_cells --cov=fold_ranker --cov=position_classifier --cov=recency_weights --cov=static_race_context --cov=margin_features --cov=event_weights --cov-config=research/jra-20260913/coverage.toml --cov-report=term-missing --cov-report="json:research/jra-20260913/logs/coverage-$RUN_ID.json" --cov-fail-under=95 research/jra-20260913/test_*.py
