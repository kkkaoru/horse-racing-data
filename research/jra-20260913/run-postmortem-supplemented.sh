#!/usr/bin/env bash
# Keep Cloudflare numeric finishes; separately diagnose official nonfinish status supplements.
set -euo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
cd "$ROOT"
export PYTHONPATH="$ROOT/research/jra-20260913"
export COVERAGE_FILE="$ROOT/research/jra-20260913/logs/review-coverage-002"
...
bash research/chronos2/run-local.sh "$PY/pytest" -p no:cacheprovider --basetemp=research/jra-20260913/test-review-002 --cov=race_rank_review --cov-config=research/jra-20260913/coverage.toml --cov-report=term-missing --cov-report=json:research/jra-20260913/logs/review-coverage-002.json --cov-fail-under=95 research/jra-20260913/test_race_rank_review.py
...
REVIEWS = []
LEDGER_PATH = ROOT/'official-status-002/status-ledger.json'
STATUS_ROWS = {row['race_id']:row for row in json.loads(LEDGER_PATH.read_text(encoding='utf-8'))}
INPUTS = [PREDICTIONS_PATH, PLAN_PATH, LEDGER_PATH]
for race in PREDICTIONS['races']:
    ...
    status_supplement = STATUS_ROWS.get(race_id)
    if status_supplement is not None:
        number = int(status_supplement['horse_number'])
        expected_name = status_supplement['horse_name']
        matching_predictions = [horse for horse in predicted if int(horse['horseNumber']) == number and horse['horseName'].strip() == expected_name]
        if number in actual or len(matching_predictions) != 1 or status_supplement['status'] not in ('中止','除外'):
            raise ValueError('Official status supplement conflicts with Cloudflare identity or outcome')
        actual[number] = None
    ...
    if status_supplement is not None:
        names_result[int(status_supplement['horse_number'])] = ''.join(unicodedata.normalize('NFKC', status_supplement['horse_name']).split())
...
SUPPLEMENTAL = [r for r in REVIEWS if r['supplemental_eligible'] and r['metrics_at_prediction'] is not None]
SUPPLEMENTAL_PAIRED = [r for r in SUPPLEMENTAL if r['metrics_at_prediction']['market_hits'] is not None]
SUMMARY['official_status_supplemented_analysis'] = {'population':'original declared prediction roster; excluded or DNF has no numeric finish; no reranking or retrospective scratch removal', 'races':len(SUPPLEMENTAL), 'paired_races':len(SUPPLEMENTAL_PAIRED), 'model_hits':[sum(r['metrics_at_prediction']['model_hits'][k] for r in SUPPLEMENTAL) for k in range(5)], 'market_delta':[sum(r['metrics_at_prediction']['model_minus_market'][k] for r in SUPPLEMENTAL_PAIRED) for k in range(5)]}
