...
from fold_ranker import RankerConfig, market_free_features, relevance_for_top5
from position_classifier import fit_position_probabilities, rank_scores_for_races
from race_rank_review import review_orders

ROOT = Path('research/jra-20260913')
OUT = ROOT / 'position-pilot-001'
...
archive.mkdir()
source_hashes = {}
for name in ('dedicated_cells.py','fold_ranker.py','position_classifier.py','race_rank_review.py','run-position-pilot.sh'):
    content = (ROOT/name).read_bytes()
    (archive/name).write_bytes(content)
    source_hashes[name] = hashlib.sha256(content).hexdigest()
(archive/'sha256.json').write_text(json.dumps(source_hashes,indent=2),encoding='utf-8')
...
for case in CASES:
    ...
    probabilities = fit_position_probabilities(x_train=x_train,finishes=np.asarray(train['actual_finish'],dtype=np.float32),abnormality=np.asarray(train['abnormality_code'],dtype=np.str_),race_ids=np.asarray(train['race_id'],dtype=np.str_),x_evaluation=x_evaluation,output=target,config=CONFIG)
    evaluation_ids = np.asarray(evaluation['race_id'],dtype=np.str_)
    scores = rank_scores_for_races(probabilities,evaluation_ids)
    np.save(target/'position-probabilities.npy',probabilities,allow_pickle=False)
    reloaded = CatBoostClassifier()
    reloaded.load_model(str(target/'model.json'),format='json')
    reloaded_probabilities = np.zeros_like(probabilities)
    reloaded_probabilities[:,np.asarray(reloaded.classes_,dtype=np.int64)] = np.asarray(reloaded.predict_proba(x_evaluation),dtype=np.float64)
    parity_error = float(np.max(np.abs(probabilities-reloaded_probabilities)))
    if not np.allclose(probabilities,reloaded_probabilities,rtol=0,atol=1e-6):
        raise ValueError('JSON export probability parity failed')
    if not np.array_equal(scores,rank_scores_for_races(reloaded_probabilities,evaluation_ids)):
        raise ValueError('JSON export full-order parity failed')
...
print('POSITION_PILOT_COMPLETE',len(SUMMARIES),'cells',flush=True)
PY
