...
out=root/'historical-features-20260912-001'
...
for venue in ('06','09'):
    for number in range(1,13):
        race=f'{number:02d}'
        key=f'feat-cache/catalog-v1/jra/20260912/{venue}/{race}/features.parquet'
        ...
        if data is not None:
            path=out/f'20260912-{venue}-{race}.parquet'
        ...
        (out/f'20260912-{venue}-{race}.receipt.json').write_text(json.dumps(receipt,indent=2,default=str),encoding='utf-8')
