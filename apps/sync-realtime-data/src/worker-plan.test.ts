...
it("isPremiumPaddockEligibleAtPlan returns false for NAR source races", async () => {
  const { isPremiumPaddockEligibleAtPlan } = await import("./worker");
  const race = buildJraRace({ source: "nar" });
  expect(
    isPremiumPaddockEligibleAtPlan({
      now: new Date("2026-05-12T05:55:00.000Z"),
      race,
      state: null,
    }),
  ).toBe(false);
...
it("isPremiumPaddockEligibleAtPlan returns true within the T-120/T+2 window with no prior state", async () => {
  const { isPremiumPaddockEligibleAtPlan } = await import("./worker");
  const race = buildJraRace({ raceStartAtJst: "2026-05-12T15:00:00+09:00" });
  expect(
    isPremiumPaddockEligibleAtPlan({
...
it("isPremiumPaddockEligibleAtPlan returns false when lastFetchAt is within the recheck window", async () => {
  const { isPremiumPaddockEligibleAtPlan } = await import("./worker");
  const race = buildJraRace({ raceStartAtJst: "2026-05-12T15:00:00+09:00" });
  expect(
    isPremiumPaddockEligibleAtPlan({
      ...
      race,
      state: {
        lastFetchAt: "2026-05-12T13:29:50+09:00",
        lastQueuedAt: null,
        retryAfter: null,
        status: "pending",
      },
...
it("planPremiumPaddockFetchesForDate gracefully drops races whose state read rejected", async () => {
  const { planPremiumPaddockFetchesForDate } = await import("./worker");
  const { listSchedulableRaceSourcesByDate, getPremiumPaddockFetchState } =
    await import("./storage");
  vi.mocked(listSchedulableRaceSourcesByDate).mockReset();
  vi.mocked(listSchedulableRaceSourcesByDate).mockResolvedValue([
    buildJraRace({ raceKey: "jra:2026:0512:08:01" }),
    buildJraRace({ raceKey: "jra:2026:0512:08:02" }),
  ]);
  vi.mocked(getPremiumPaddockFetchState).mockReset();
  vi.mocked(getPremiumPaddockFetchState)
    .mockRejectedValueOnce(new Error("D1 overloaded"))
    .mockResolvedValueOnce(null);
  const result = await planPremiumPaddockFetchesForDate(
    buildEnv({ PREMIUM_RACE_ORIGIN: "https://x.test" }),
    "20260512",
    new Date("2026-05-12T05:30:00.000Z"),
  );
  expect(result).toHaveLength(1);
  const first = result[0]!;
  expect(first.type === "fetch-premium-paddock" ? first.raceKey : null).toBe("jra:2026:0512:08:02");
});

it("resolvePremiumPaddockSkipKind returns auth_required when authRequired is true", async () => {
  const { resolvePremiumPaddockSkipKind } = await import("./worker");
  expect(
    resolvePremiumPaddockSkipKind({
      authRequired: true,
      bulletins: [],
      pending: false,
      unavailable: false,
...
it("resolvePremiumPaddockSkipKind returns unavailable when unavailable is true and auth is false", async () => {
  const { resolvePremiumPaddockSkipKind } = await import("./worker");
  expect(
    resolvePremiumPaddockSkipKind({
      authRequired: false,
...
it("resolvePremiumPaddockSkipKind returns pending when pending is true and earlier flags are false", async () => {
  const { resolvePremiumPaddockSkipKind } = await import("./worker");
  expect(
    resolvePremiumPaddockSkipKind({
      authRequired: false,
...
it("resolvePremiumPaddockSkipKind returns empty when all flags are false and bulletins is empty", async () => {
  const { resolvePremiumPaddockSkipKind } = await import("./worker");
  expect(
    resolvePremiumPaddockSkipKind({
      authRequired: false,
...
it("resolvePremiumPaddockSkipKind returns null when bulletins are present", async () => {
  const { resolvePremiumPaddockSkipKind } = await import("./worker");
  expect(
    resolvePremiumPaddockSkipKind({
      authRequired: false,
      bulletins: [{ horseNumber: "1" }],
      pending: false,
      unavailable: false,
    }),
  ).toBeNull();
