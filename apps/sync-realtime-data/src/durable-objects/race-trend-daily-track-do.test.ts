...
import type { TrainerFetchArgs } from "./race-trend-trainer-fetch";
...
// Trainer enrichment tests: 2026-06-06 Merge A (race-trend trainer) needs
// today-sibling rows to carry chokyoshiName. The DO's D1 base table
// (race_entry_snapshots) has no trainer column so refreshFromD1 runs the
// Hyperdrive trainer fetcher in parallel and merges the result by
// (raceBango, umaban) key.

const TRAINER_SNAPSHOT_ROW: FakeD1ResultRecord = {
  changeAmount: null,
  changeSign: null,
  expectedHorseCount: 2,
  fetchedAt: "2026-06-06T11:05:00+09:00",
  finishPosition: "1",
  hassoJikoku: "2026-06-06T11:00:00+09:00",
  horseName: "TrainerHorseA",
  jockeyName: "JockeyA",
  kaisaiNen: "2026",
  kaisaiTsukihi: "0606",
  keibajoCode: "06",
  raceBango: "03",
  raceKey: "jra:2026:0606:06:03",
  raceName: "Sibling 03",
  resultCompleteAt: "2026-06-06T11:05:00+09:00",
  savedHorseCount: 2,
  sohaTime: "1:34.0",
  source: "jra",
  umaban: "1",
...
const TRAINER_SNAPSHOT_ROW_TWO: FakeD1ResultRecord = {
  ...TRAINER_SNAPSHOT_ROW,
  horseName: "TrainerHorseB",
  jockeyName: "JockeyB",
  umaban: "2",
};

it("GET /races cold-start enriches starter rows with trainer names via the Hyperdrive fetcher", async () => {
  const handle = buildFakeState(new Map());
  const env = buildEnv({ snapshotResults: [TRAINER_SNAPSHOT_ROW, TRAINER_SNAPSHOT_ROW_TWO] });
  const trainerFetcher = vi.fn(
  ...
  const cache = await RaceTrendDailyTrackDO.createForTest({
    env,
    state: handle.state,
    trainerFetcher,
  });
  const response = await cache.fetch(
    new Request(
      "https://race-trend-daily-track-do/races?source=jra&ymd=20260606&keibajo=06&beforeRaceBango=99",
    ),
  );
  expect(response.headers.get("X-Race-Trend-DO")).toBe("hit");
  const payload = (await response.json()) as { races: RaceTrendDailyTrackRow[] };
  expect(payload.races[0]!.starterRows.map((row) => row.chokyoshiName)).toStrictEqual([
    "TrainerNameA",
    "TrainerNameB",
  ]);
});

it("GET /races cold-start passes the unique sorted raceBango list to the trainer fetcher", async () => {
  const SIBLING_ROW: FakeD1ResultRecord = {
    ...TRAINER_SNAPSHOT_ROW,
    raceBango: "05",
    raceKey: "jra:2026:0606:06:05",
  };
  const handle = buildFakeState(new Map());
  const env = buildEnv({
    snapshotResults: [TRAINER_SNAPSHOT_ROW, TRAINER_SNAPSHOT_ROW_TWO, SIBLING_ROW],
  });
  const recordedFetcherArgs: Array<{
    parsed: { source: string; targetYmd: string; keibajoCode: string };
    raceBangoList: ReadonlyArray<string>;
  }> = [];
  const trainerFetcher = async (args: TrainerFetchArgs): Promise<Map<string, string>> => {
    recordedFetcherArgs.push({
      parsed: {
        keibajoCode: args.parsed.keibajoCode,
        source: args.parsed.source,
        targetYmd: args.parsed.targetYmd,
      },
      raceBangoList: args.raceBangoList,
    });
    return new Map();
  };
  const cache = await RaceTrendDailyTrackDO.createForTest({
  ...
  expect(recordedFetcherArgs).toHaveLength(1);
  expect(recordedFetcherArgs[0]!.raceBangoList).toStrictEqual(["03", "05"]);
  expect(recordedFetcherArgs[0]!.parsed).toStrictEqual({
    keibajoCode: "06",
    source: "jra",
    targetYmd: "20260606",
  });
...
it("GET /races partial trainer match leaves the missing horse's chokyoshiName as null", async () => {
  const handle = buildFakeState(new Map());
  const env = buildEnv({ snapshotResults: [TRAINER_SNAPSHOT_ROW, TRAINER_SNAPSHOT_ROW_TWO] });
  const trainerFetcher = vi.fn(
    async (): Promise<Map<string, string>> => new Map([["03:1", "TrainerNameA"]]),
  );
  const cache = await RaceTrendDailyTrackDO.createForTest({
    env,
    state: handle.state,
...
it("GET /races falls back to null trainer when the fetcher returns an empty Map (timeout / disabled)", async () => {
  const handle = buildFakeState(new Map());
  const env = buildEnv({ snapshotResults: [TRAINER_SNAPSHOT_ROW, TRAINER_SNAPSHOT_ROW_TWO] });
  const trainerFetcher = vi.fn(async (): Promise<Map<string, string>> => new Map());
  const cache = await RaceTrendDailyTrackDO.createForTest({
    env,
    state: handle.state,
    trainerFetcher,
  ...
  const payload = (await response.json()) as { races: RaceTrendDailyTrackRow[] };
  expect(payload.races[0]!.starterRows.map((row) => row.chokyoshiName)).toStrictEqual([null, null]);
});

it("__testables.collectRaceBangoList returns sorted unique raceBango values", () => {
  const filtered = [
    { ...TRAINER_SNAPSHOT_ROW, raceBango: "05" },
    { ...TRAINER_SNAPSHOT_ROW, raceBango: "03" },
    { ...TRAINER_SNAPSHOT_ROW, raceBango: "05" },
    { ...TRAINER_SNAPSHOT_ROW, raceBango: "01" },
  ].filter(__testables.isRawSnapshotRow);
  expect(__testables.collectRaceBangoList(filtered)).toStrictEqual(["01", "03", "05"]);
});

it("__testables.collectRaceBangoList returns an empty array for no rows", () => {
  expect(__testables.collectRaceBangoList([])).toStrictEqual([]);
});

it("__testables.decorateWithTrainer attaches the trainer name when the lookup hits", () => {
  const filtered = [TRAINER_SNAPSHOT_ROW].filter(__testables.isRawSnapshotRow);
  const decorated = __testables.decorateWithTrainer(
    filtered[0]!,
    new Map([["03:1", "TrainerNameA"]]),
  );
  expect(decorated.chokyoshiName).toBe("TrainerNameA");
});

it("__testables.decorateWithTrainer leaves chokyoshiName as null when the lookup misses", () => {
  const filtered = [TRAINER_SNAPSHOT_ROW].filter(__testables.isRawSnapshotRow);
  const decorated = __testables.decorateWithTrainer(filtered[0]!, new Map());
  expect(decorated.chokyoshiName).toBeNull();
});

it("buildRowsFromSnapshotResults forwards chokyoshiName from raw row into starter row", () => {
  const rawRow: FakeD1ResultRecord = { ...TRAINER_SNAPSHOT_ROW, chokyoshiName: "PreSetTrainer" };
  const filtered = [rawRow].filter(__testables.isRawSnapshotRow);
  const rows = __testables.buildRowsFromSnapshotResults(filtered, []);
  expect(rows[0]!.starterRows[0]!.chokyoshiName).toBe("PreSetTrainer");
});

it("toStarterRow coalesces a missing chokyoshiName to null (back-compat with un-decorated rows)", () => {
  const filtered = [TRAINER_SNAPSHOT_ROW].filter(__testables.isRawSnapshotRow);
  const rows = __testables.buildRowsFromSnapshotResults(filtered, []);
  expect(rows[0]!.starterRows[0]!.chokyoshiName).toBeNull();
});
