    ...
    rows: [
      {
        ...
        default_transaction_read_only_source: "session",
...
it("runs a non-mutating SELECT with no bind parameters", async () => {
  ...
  expect(typeof sql === "string" && sql.indexOf("default_transaction_read_only_source") > -1).toBe(
    true,
  );
  expect(
    typeof sql === "string" &&
      sql.indexOf("pg_settings.source from pg_settings where pg_settings.name") > -1,
  ).toBe(true);
  expect(typeof sql === "string" && sql.indexOf("sourcefile") > -1).toBe(false);
  expect(typeof sql === "string" && sql.indexOf("pg_db_role_setting") > -1).toBe(false);
  expect(typeof sql === "string" && sql.indexOf("setconfig") > -1).toBe(false);
...
it("reports pg_settings.source for default_transaction_read_only", async () => {
  queryMock.mockResolvedValue({
    rows: [
      {
        can_insert_finish_position: true,
        ...
        can_update_running_style: true,
        default_transaction_read_only: true,
        default_transaction_read_only_source: "database",
        fp_table_present: true,
        in_recovery: false,
        rs_table_present: true,
        transaction_read_only: true,
...
it("classifies a non-string default_transaction_read_only_source as unknown", async () => {
  queryMock.mockResolvedValue({
    rows: [
      {
        can_insert_finish_position: true,
        ...
        can_update_running_style: true,
        default_transaction_read_only: false,
        default_transaction_read_only_source: 1,
        fp_table_present: true,
        in_recovery: false,
        rs_table_present: true,
        transaction_read_only: false,
...
it("classifies a missing default_transaction_read_only_source as unknown", async () => {
  queryMock.mockResolvedValue({
    rows: [
      {
        can_insert_finish_position: true,
