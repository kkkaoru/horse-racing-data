...
//
// Phase F final step for daily_race_entries cleanup. Deletes legacy
// daily_race_entries rows (rowid > resume cursor) from the old
// sync-realtime-data D1 via POST /api/internal/delete-daily-race-entries-chunk
...
// is persisted to KV `features:cleanup:daily-race-entries-cursor` through the
// new features worker's /api/internal/migration-state endpoint.
//
// Old-worker D1 daily_race_entries writes have already been stopped at this
...
const CURSOR_KEY = "features:cleanup:daily-race-entries-cursor";

export interface DeleteDailyRaceEntriesChunkResponse {
  deletedRowCount: number;
  nextSinceRowid: number;
}

export interface DeleteDailyRaceEntriesChunkConfig {
  oldWorkerUrl: string;
...
export interface DeleteDailyRaceEntriesChunkResult {
  totalDeleted: number;
...
const deleteChunkOnce = async (
  config: DeleteDailyRaceEntriesChunkConfig,
  sinceRowid: number,
): Promise<DeleteDailyRaceEntriesChunkResponse> => {
  const response = await config.fetchImpl(
    `${config.oldWorkerUrl}/api/internal/delete-daily-race-entries-chunk`,
    {
      body: JSON.stringify({
  ...
  if (!response.ok) {
    throw new Error(`delete-daily-race-entries-chunk failed: ${response.status}`);
  }
  return (await response.json()) as DeleteDailyRaceEntriesChunkResponse;
};
...
const loadResumeCursor = async (config: DeleteDailyRaceEntriesChunkConfig): Promise<number> => {
  const response = await config.fetchImpl(
...
export const deleteDailyRaceEntriesChunked = async (
  config: DeleteDailyRaceEntriesChunkConfig,
): Promise<DeleteDailyRaceEntriesChunkResult> => {
  ...
  while (true) {
    ...
    let chunk: DeleteDailyRaceEntriesChunkResponse;
    try {
