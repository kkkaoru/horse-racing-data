...
このパッケージは Cloudflare Workers (D1 / R2 / Queue / Durable Objects) を多用するため、 v8 が `??` / `||` / `&&` / 三項演算子を 1 分岐ずつ計測する特性上、 Branches は dead defensive arm を残すと下がりやすい。 以下のしきい値を **`vitest.config.ts`** で固定しています。
...
| **Branches**   | **>= 95%** | dead defensive arm (`?? null` 等) を残さないこと |
...
### Branches を維持・上げる時の正攻法

優先順位 (95% を割ったら復帰用、 96%+ に上げたい時の継続用):

1. **dead defensive arms を削除する** — `match[1] ?? ""` のように regex matchAll 後の capture group は常に存在するので `match[1]!` に。 1 つ削るだけで Branches 分母 -2 / カバー率 +0.03〜0.06pp。 `refactor(realtime): drop dead defensive ?? ...` のような単一目的コミットにする。
2. **`?? null` / `?? defaultValue` の else arm をテストで通す** — 入力に null を含めるバリエーションを追加。
3. **多腕 ternary (`a ? x : b ? y : z`)** はそれぞれの腕を 1 ケース以上テストする。
4. **`if (import.meta.main)` CLI wrapper のみ `/* v8 ignore next N */` で除外を許可** — vitest からは構造上テスト不可能。 他用途での `v8 ignore` は禁止 (CLAUDE.md `exclude` 禁止規定とグレーゾーン)。
5. 上記で十分なテストを書いた上でしきい値を **95 → 96 → 97…** と段階的に上げる。 下げる方向は禁止。
