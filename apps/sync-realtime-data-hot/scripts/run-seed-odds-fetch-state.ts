// Run with: bun run apps/sync-realtime-data-hot/scripts/run-seed-odds-fetch-state.ts
...
import { buildDefaultConfig, seedOddsFetchState } from "./seed-odds-fetch-state";
...
const result = await seedOddsFetchState({ ...config, fetchImpl: resolvingFetch });
console.log(
  `seed-odds-fetch-state done: seeded=${result.totalSeeded}, maxRowid=${result.maxRowid}`,
);
