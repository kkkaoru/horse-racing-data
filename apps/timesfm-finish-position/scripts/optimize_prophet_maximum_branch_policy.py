"""Optimize Prophet weights for every maximum-granularity production branch."""
...
from timesfm_finish_position.policy_report import (
    MetricReport,
    SignatureCategoryReport,
    SignatureCellReport,
    SignatureEvaluationReport,
    require_category,
    require_float,
    require_int,
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        "prophet-maximum-branch-weight-optimization-2024-2026.json",
    )
...
) -> tuple[dict[tuple[str, str, str, str], BranchEvidence], Counter[str]]:
    trends = load_trends(paths)
    lookup = load_lookup(paths.lookup)
    evidence: dict[tuple[str, str, str, str], BranchEvidence] = {}
        ...
        for category in CATEGORIES:
            for year in YEARS:
                ...
                for (race_id, entries), rows in zip(races.items(), scored, strict=True):
                    ...
                    cell = cells[race_id]
                    branch = str(rows[0][0])
                    signature = served_signatures[race_id]
                    race_evidence, reason = linear_race(rows, entries, category, cell)
                    key = (category, cell, branch, signature)
                    state = evidence.setdefault(
                        key, BranchEvidence(category, cell, branch, signature)
                    )
...
def build_report(
    ...
    for category in CATEGORIES:
        ...
        branch_reports = [
            branch for cell in cells.values() for branch in cell["signatures"].values()
        ]
        ...
        categories[category] = {
            ...
            "summary": {
                ...
                "maximum_branches": len(branch_reports),
                "races": race_count,
    ...
    return {
        "contract": {
            "years": [2024, 2025, 2026],
            "unit": (
                "resolved cell plus routing mode, stage-2 decision outcome/model, "
                "Stage-1 gate outcome, and final served model/composite version"
            ),
...
def main() -> None:
    ...
    summary = {
        category: sum(
            len(cell["signatures"]) for cell in report["categories"][category]["cells"].values()
        )
        for category in CATEGORIES
    }
    print(json.dumps({"output": str(paths.output), "maximum_branches": summary}, indent=2))
