...
### 本番デプロイ・実データ検証 (2026-07-18)

commit `a26d14b8` を deploy、**Version ID
`123308ef-ac25-42d4-9035-e6090b259639`**。デプロイ前にキャッシュ2層の
namespace を `v4`→`v5`（query-cache.ts）/ `v3`→`v4`
（finish-prediction-inputs-cache.server.ts）へ再 bump 済みなので、
本日既に読み込まれていたレースのページも即座に新形状へ反映される。

**本日の garbage レース** (函館1R、2026-07-18、
`/races/2026/07/18/02/01`、predicted_score stddev 実測 0.087) —
chrome-devtools で確認:

- 順位テーブルは**デフォルトで表示されたまま** (8 行、非表示なし)。
- 確信度 badge「予測の自信度: 低」表示は継続。
- 新規表示ブロック「予測スコアのばらつき (標準偏差): 0.09」+ 説明文が
  常設表示。
- 標準偏差が閾値 0.5 未満のため、警告文
  「この数値は健全なレースの分布と比べて低く、予測の信頼性が低い可能性
  があります。」+「この予測を非表示にする」チェックボックス (既定
  unchecked) が追加表示。
- **インタラクション確認**: チェックボックスをクリックすると、順位
  テーブルと補正トグル UI が「選択により、この予測の順位表を非表示に
  しています。上のチェックボックスを外すと再表示できます。」に即座に
  差し替わることを実際にクリックして確認 (checked: true, table:
  非表示)。

**健全な対照レース** (2026-06-21 阪神1R、predicted_score stddev
実測 1.46、`/races/2026/06/21/09/01`) — 同じ UI で
「予測スコアのばらつき (標準偏差): 1.46」が表示され、警告文・
チェックボックスは表示されず、16 行の順位テーブルが通常どおり表示
されることを確認。標準偏差の生値表示は閾値に関わらず常に同じ形式で
出るという設計どおりの挙動を実データで確認済み。
