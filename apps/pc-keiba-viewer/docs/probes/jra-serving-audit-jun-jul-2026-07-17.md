...
- **対処状況**: **Worker/DO 層は修正済み・deploy 済み（同日 07-18 09:1x-09:18 JST、USER 承認により当初想定の17:00待ちから前倒し）**。実装は上記案 1+2 を両方満たす形（`force` を `claimFocusedFullRace` シグネチャに追加し `force===true` のとき `TERMINAL_STATUSES` チェックをスキップ、そのまま既存の「新規キー」overwrite パスに合流させることで実質的な reset も同時に達成）。`do-state.ts`/`predict-run-coordinator.ts`/`queue-consumer.ts`/`types.ts` の4ファイル+対応テスト、commit `78076cc8`（654 tests pass、cov 99.64/96.84/100/99.77、lint 0、tsc 0）。Worker deploy version `12344a51`（09:18 JST）。その後 ⑪ race-sharded DO 化 (`RACE_SHARDED_DO=1`) との bundle deploy `84e51027`（09:20:35 JST、bundle owner: do-sharding）に乗り換え済み。**Worker/DO 層単独の解消は本番 smoke で確認済み**: 同一レース (Hakodate 2026-07-12 R01) への再 force トリガーで `wrangler tail` 上 `focused-claim ... proceed=true state=-`（旧: `proceed=false state=success`）を直接確認、`doName=predict-jra-1`（sharding 経路）に正しくルーティングされた。**ただし end-to-end (Neon への実際の健全行着地) は別の第4のゲートによりまだブロックされたまま — Defect H 参照。72 レース修復の再開には Defect H の修正も必要。**
...
### Defect H (2026-07-18 発見・現在進行形): Python container 側にも独立した row-count-only completion gate があり、Defect F の Worker/DO 層修正だけでは 72 レース修復の end-to-end は解消しない

- **発見の経緯**: Defect F の Worker/DO 層修正 (`78076cc8`+bundle deploy `84e51027`) 後、Hakodate 2026-07-12 R01 への同一 force リトリガーを `wrangler tail` で追跡したところ、`focused-claim ... proceed=true state=-`（Defect F の修正が正しく効いている証拠）まで進んだ直後、`container-result ... status=already-complete racesPredicted=0` / `Focused full already complete (container)` で早期終了した。Neon 側もこの後 30 分ポーリングしても行は更新されず（`prediction_generated_at` は元の 2026-07-12T05:52:09Z のまま）、Defect F 単体では end-to-end の解消に至らないことを確認した。
- **コード根拠**: Cloudflare Worker 側 (`queue-consumer.ts`) の claim/completion gate とは完全に独立した第4のゲートが、Python container 内 (`apps/finish-position-predict-container/src/predict_lib/serve.py::_focused_full_preflight` → `_focused_full_is_complete`) に存在する。`completion_fn`（実体は `predict_upcoming.py::_make_focused_full_completion_fn` が返す `_focused_full_prediction_complete`）は `race_entry_corner_features` の distinct `ketto_toroku_bango` 数（期待頭数）と `race_finish_position_model_predictions` の該当 `model_version` 実測行数を比較するのみ (`actual_rows >= len(expected_horses)`、`predict_upcoming.py:1861-1989`) — Defect F 修正前の TypeScript 側 `ackIfFocusedFullAlreadyComplete` と全く同じ「行数だけを見る」アンチパターンを Python 側で独立に再実装している。**`PredictParams`（`serve.py`、`__slots__` 参照）に `force` フィールド自体が存在せず**、`buildPredictUrl` / `buildFocusedFullPredictUrl`（両方とも `queue-consumer.ts` / `worker.ts`）のどちらも `/predict` へのクエリ文字列に `force` を一切含めていない — Worker 側でどれだけ `force=true` を扱っても、Container に届く時点でその情報は完全に失われている。
- **do-sharding の today's-race テストが成功し、自分の historical repair race が失敗した理由**: 前者は当日新規レースで `race_finish_position_model_predictions` に既存行が 0 件のため `0 >= len(expected_horses)` が偽となり素通りしてパイプラインが走った。後者 (Defect A の 72 レース) は既に (壊れた) 行が期待頭数分揃っているため、この box-count check が無条件で `true` を返し、`force` の有無に関わらず re-score 自体が一度も起動しない。
- **影響範囲**: Defect F 修正後も、`race_entry_corner_features` の期待頭数と一致する行数を既に持つ全レース（Defect A の 72 レースはほぼ全件がこれに該当）への force re-trigger は、Worker 層を正しく通過した後、Container 層で無条件にスキップされる。72 レース修復はこの第4ゲートの修正なしには実行不能。
- **本番 live serving への影響: 低い**（Defect F と同様の理由 — 当日の新規レースは行数 0 から始まるため本チェックには最初から抵触しない。coordinator の `mode=rescore` 経路も本チェックを経由しない別コード）。
- **対処状況**: **未修正（提案のみ、team-lead 報告済み・実装可否は指示待ち）**。修正案: `force` を query param として `buildPredictUrl`/`buildFocusedFullPredictUrl`（TS）に足し、Python 側 `PredictParams` に `force: bool = False` を追加（`__slots__` にも追加）、`_focused_full_preflight` で `params.force` が真なら `_focused_full_is_complete` の呼び出し自体をスキップする。TypeScript (`finish-position-cron`) と Python (`finish-position-predict-container`、既存 cov 99.81% を回帰させない同等のテスト追加が必要) の両パッケージにまたがる変更で、Defect F より変更範囲が広い。開催中の cross-package 変更は Defect F 以上に慎重な判断を要するため、実装は team-lead/USER 判断待ちとし、本監査ではコード変更は行っていない。
