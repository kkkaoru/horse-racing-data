...
it("keeps partnership and venue statistics independent of same-condition rows", () => {
  ...
  expect(rows[0]?.cells.horseJockey.winRate).toBe(20);
  expect(rows[0]?.cells.jockeyVenue.starts).toBe(100);
  expect(rows[0]?.cells.jockeyTrainerVenue.winRate).toBe(0);
  expect(rows[0]?.cells.jockey.winRate).toBeNull();
});
...
it("places 騎手枠別 immediately left of 騎手 in the heatmap column order", () => {
  ...
  expect(WIN_RATE_HEATMAP_COLUMNS[5]).toStrictEqual({ key: "jockeyFrame", label: "騎手枠別" });
  expect(WIN_RATE_HEATMAP_COLUMNS[6]).toStrictEqual({ key: "jockey", label: "騎手" });
});
...
it("keeps per-umaban jockeyFrame rates when two horses share a jockey in different frames", () => {
  expect(
    buildWinRateHeatmapRows({
      ...
      horseJockey: row.cells.horseJockey,
      jockeyVenue: row.cells.jockeyVenue,
      jockeyTrainerVenue: row.cells.jockeyTrainerVenue,
    })),
  ).toStrictEqual([
    {
      horseNumber: "1",
