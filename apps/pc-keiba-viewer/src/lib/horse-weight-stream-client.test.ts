...
it("keeps the stream open and applies a corrected non-empty snapshot", () => {
  const { result } = renderHook(() =>
    useHorseWeightStream({
      day: "30",
      initial: null,
  ...
  act(() => {
    instance.dispatchWeightsEvent(
      JSON.stringify({
        fetchedAt: "2026-05-30T08:34:00.000Z",
        horses: [
          { changeAmount: 2, changeSign: "+", horseName: "Beta", horseNumber: "2", weight: 500 },
        ],
      }),
    );
  });
