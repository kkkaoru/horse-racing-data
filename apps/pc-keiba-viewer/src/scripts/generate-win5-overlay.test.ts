...
test("buildOverlayModelVersion appends rs-overlay suffix", () => {
  expect(buildOverlayModelVersion("20260524")).toBe(
    "win5-xgb-v7-lineage-v1-rs-overlay-20260524",
  );
});
...
test("formatTodayYyyymmddJst formats a known UTC instant into JST date", () => {
  ...
  expect(formatTodayYyyymmddJst(new Date(Date.UTC(2026, 4, 23, 18, 0, 0)))).toBe(
    "20260524",
  );
});
...
test("formatTodayYyyymmddJst handles JST midnight rollover", () => {
  ...
  expect(formatTodayYyyymmddJst(new Date(Date.UTC(2026, 4, 23, 14, 59, 59)))).toBe(
    "20260523",
  );
  // 2026-05-23T15:00:00Z = 2026-05-24T00:00:00+09:00 → JST date is 20260524
  expect(formatTodayYyyymmddJst(new Date(Date.UTC(2026, 4, 23, 15, 0, 0)))).toBe(
    "20260524",
  );
});
