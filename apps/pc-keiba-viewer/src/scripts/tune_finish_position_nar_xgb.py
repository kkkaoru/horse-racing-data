#!/usr/bin/env python3
"""Iter 12 (L3): Optuna HPO for NAR XGBoost on iter 9 pacestyle features.
...
- ``max_depth in [5, 10]`` int
- ``learning_rate in [0.04, 0.08]`` log (floor 0.04)
- ``reg_lambda in [1.0, 3.0]`` log (floor 1.0)
- ``min_child_weight in [1, 10]`` int
- ``subsample in [0.6, 1.0]`` float
- ``colsample_bytree in [0.6, 1.0]`` float
- ``n_estimators in [400, 800]`` int
...
REG_LAMBDA_FLOOR: Final[float] = 1.0
MAX_DEPTH_CEIL: Final[int] = 10
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="tune_finish_position_nar_xgb")
...
def load_year_parquet(root: Path, year: int) -> pd.DataFrame | None:
...
def assert_no_race_overlap(train_df: pd.DataFrame, valid_df: pd.DataFrame) -> None:
    train_races = set(train_df["race_id"].astype(str).tolist())
    valid_races = set(valid_df["race_id"].astype(str).tolist())
...
def build_fold_frames(
    ...
    train_df = train_df[train_df["finish_position"].notna()].copy()
    valid_df = valid_df[valid_df["finish_position"].notna()].copy()
...
def attach_bucket_keys(
    valid_df: pd.DataFrame, bucket_df: pd.DataFrame | None,
) -> list[str]:
    if bucket_df is None:
        return ["__all__"] * int(valid_df["race_id"].nunique())
    ...
    if key_col is None:
        return ["__all__"] * int(valid_df["race_id"].nunique())
    bucket_keyed = bucket_df[["race_id", key_col]].drop_duplicates(subset=["race_id"])
    race_order = valid_df["race_id"].drop_duplicates().tolist()
    lookup = dict(
        zip(
            bucket_keyed["race_id"].astype(str).tolist(),
            bucket_keyed[key_col].astype(str).tolist(),
            strict=False,
        ),
    )
...
def suggest_params(trial: TrialLike) -> dict[str, object]:
    return {
        "max_depth": trial.suggest_int("max_depth", 5, MAX_DEPTH_CEIL),
        "learning_rate": trial.suggest_float(
            "learning_rate", LR_FLOOR, 0.08, log=True,
        ),
        "reg_lambda": trial.suggest_float(
            "reg_lambda", REG_LAMBDA_FLOOR, 3.0, log=True,
        ),
...
def enforce_stability_floor(params: "Mapping[str, object]") -> dict[str, object]:
    ...
    rl_raw = cast(float, merged.get("reg_lambda", REG_LAMBDA_FLOOR))
    merged["reg_lambda"] = max(float(rl_raw), REG_LAMBDA_FLOOR)
    md_raw = cast(int, merged.get("max_depth", MAX_DEPTH_CEIL))
    merged["max_depth"] = min(int(md_raw), MAX_DEPTH_CEIL)
    return merged
...
def train_xgb_fold(
    ...
    train_labels = train_df["finish_position"].map(to_relevance).to_numpy(
        dtype=np.int32,
    )
    valid_labels = valid_df["finish_position"].map(to_relevance).to_numpy(
        dtype=np.int32,
    )
    ...
    booster = xgb.train(
        xgb_params,
        dtrain,
        num_boost_round=int(cast(int, params["n_estimators"])),
        evals=[(dvalid, "valid")],
    ...
    return np.asarray(
        booster.predict(dvalid, iteration_range=(0, best_iter + 1)),
        dtype=np.float64,
    )
...
class FoldDeps:
    ...
    train_fn: "Callable[[pd.DataFrame, pd.DataFrame, list[str], Mapping[str, object], int], np.ndarray]"
...
def evaluate_params(
    ...
    for held_out in cv_years:
        ...
        labels = frames.valid_df["finish_position"].map(to_relevance).to_numpy(
            dtype=np.int32,
        )
...
def run_study(args: TuneArgs) -> dict[str, object]:
    ...
    deps = FoldDeps(fold_frames=fold_frames_loader, train_fn=train_xgb_fold)
