...
def load_bucket_year(root: Path, year: int) -> pd.DataFrame | None:
    year_dir = root / f"category=jra" / f"race_year={year}"
    if not year_dir.exists():
        return None
    files = sorted(glob.glob(str(year_dir / "*.parquet")))
    if not files:
...
def train_cb_fold(
    ...
    valid_features = valid_df[feature_cols].astype(np.float32)
    train_groups = train_df["race_id"].astype("category").cat.codes.to_numpy()
    valid_groups = valid_df["race_id"].astype("category").cat.codes.to_numpy()
    train_pool = Pool(
