...
const isUsableTrendDetail = (detail: RaceTrendDetail): detail is UsableTrendDetail =>
  detail.finishPosition > 0 && detail.horseNumber !== null;

const buildTrendFinishPositionByHorse = (
  trendPayload: RaceTrendPayload | null,
): Map<string, number> => {
  if (!trendPayload) {
    return new Map<string, number>();
  }
  const entries = trendPayload.runningStyleRows.flatMap((row) =>
    row.details.filter(isUsableTrendDetail).map(toTrendFinishEntry),
  );
...
export async function RaceDetailView({
  ...
  // DISPLAY range collapses to today only — the user wanted today's sibling
  // races to be the first thing visible on the panel. R1 is the first race
  // of the day at a venue so it has no today siblings yet — we keep the
  // past-14-days default for it. The cache layer's 14-day fetch is
        ...
        <section className="runners-section">
          ...
          {runners.length === 0 ? (
            ...
            <RunnersTable
              ...
              runners={runners}
              trendFinishPositionByHorse={trendFinishPositionByHorse}
            />
          )}
        </section>
