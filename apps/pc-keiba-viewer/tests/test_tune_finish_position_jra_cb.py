"""Unit tests for ``tune_finish_position_jra_cb`` (Iter 13 Optuna HPO).

NOT-DRY: each test is fully self-contained. Mocks Optuna ``Trial``,
catboost ``CatBoost`` + ``Pool``, and filesystem I/O. No real disk reads beyond per-test
``tmp_path`` writes.
"""
...
def test_load_bucket_year_empty_dir(tmp_path: Path) -> None:
    (tmp_path / "category=jra" / "race_year=2024").mkdir(parents=True)
    assert mod.load_bucket_year(tmp_path, 2024) is None
...
def test_suggest_params_returns_all_expected_keys() -> None:
    trial = MagicMock()
    # depth, iterations
    trial.suggest_int.side_effect = [7, 600]
    # lr, l2, bagging_temp, random_strength
    trial.suggest_float.side_effect = [0.05, 5.0, 0.5, 1.0]
...
def test_enforce_stability_floor_clamps_l2() -> None:
    out = mod.enforce_stability_floor({"l2_leaf_reg": 1.0})
    assert out["l2_leaf_reg"] == mod.L2_FLOOR


def test_enforce_stability_floor_clamps_depth() -> None:
    out = mod.enforce_stability_floor({"depth": 15})
    assert out["depth"] == mod.DEPTH_CEIL
...
def test_train_cb_fold_invokes_cb_and_returns_array() -> None:
    ...
    fake_model.predict.return_value = np.array([0.5, 0.4])
    with patch("catboost.CatBoost", return_value=fake_model) as cb_mock, \
         patch("catboost.Pool") as pool_mock:
        pool_mock.return_value = MagicMock()
        result = mod.train_cb_fold(
...
def test_evaluate_params_aggregates_across_folds() -> None:
    ...
    g, w = mod.evaluate_params(
        {"depth": 5}, (2024,), 42, deps, ["feature_a"],
    )
...
def test_run_study_writes_outputs(tmp_path: Path) -> None:
    ...
    with patch.object(mod, "train_cb_fold", side_effect=fake_train):
...
def test_run_study_unions_feature_columns_across_all_cv_years(tmp_path: Path) -> None:
    features_root = tmp_path / "feats"
    bucket_root = tmp_path / "bucket"
    # Year 2022 lacks feature_c; years 2023 and 2024 have it.
    # With cv_years[0]-only discovery, feature_c would be missed.
    # With all-years union discovery, feature_c is included and used in the
    # fold where train=2022+2023 and valid=2024 (both sides have the column).
    ...
    with patch.object(mod, "train_cb_fold", side_effect=fake_train):
        mod.run_study(args)
    # fold with train=[2022,2023] and valid=2024 must discover feature_c
