...
2. `cell_metrics.json` の各 `trained_cells[*].metrics` は rate だけでなく、`prediction_count`、`top2_hit_count`、`confusion_matrix`、`per_class_log_loss_sum/count` を含む。cell 間・期間間の集計は rate 平均ではなく raw count / sum から再計算する。
3. `trained_cells[*].cell_training_evaluation` は `cell_training_evaluations.prediction_target = 'running_style'` に投入するための互換 mapping を持つ。`train-cells` 単体では Neon へ upsert しないため、永続化は別途 import/upsert step で行う。
...
5. 採用 cell だけを feature-selection routing JSON に残す。variant には `feature_set_hash` と `feature_names` を含める。
