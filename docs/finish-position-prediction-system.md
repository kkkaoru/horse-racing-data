...
`sync-realtime-data` に D1 migration `0032_running_style_predicted_corner.sql` を remote apply し、Worker Version `715d70e5-c0b8-4f50-89bf-eb74e44c02ff` で running-style の `predicted_corner_front_score` / `predicted_corner_rank` 永続化を production deploy 済み。production smoke は `nar:20260703:43:12` を本番 Worker `/api/jobs/run-inline` の `generate-running-style-predictions` で実行し、D1 `race_running_styles` は 11/11 行で score/rank が non-null（rank 1..11）、Neon `race_running_style_model_predictions` も 11/11 行で score/rank が non-null になった。`fetch_logs` は `cellModelKey="running-style/models/nar/latest.flatbin"` / `cellVariantId="latest"` / `neonWrittenCount=11` / `finishPositionTriggerMode="queue"` を記録し、running-style completion 後に finish-position generation queue へ進むことも確認済み。completed 済み旧行は `listRaceRunningStylesForRace` が score/rank を導出し、`cacheAndSyncCompletedRunningStyles` が D1 へ再 UPSERT してから viewer cache / Neon sync を行うため、旧 null 行も再同期時に補完される。
