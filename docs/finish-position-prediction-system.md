...
> **重要な留保・更新（2026-07-02 investigation → 同日中に解決、詳細は §5.9）**: 本欄は当初、「本番の per-race パイプラインが実際にこれらのモデルで着順予測を完走し Neon に書き込んだという確認済みの証跡が、テーブル観測可能な履歴の範囲（2026-05-15 以降、約 1.5 ヶ月）で 3 カテゴリいずれについても存在しない」という critical finding を記録していた。**同日中の root-cause 特定と fix（commit `2d3535be` instrumentation → commit `af1ca40e` fix）により、JRA については live smoke test で genuine な Neon 書き込みを確認済み**（`jra:2026:0628:10:01`、`race_finish_position_model_predictions` に `model_version='jra-cb-v9-sim-2013'` の 18 行、`prediction_generated_at=2026-07-02T00:09:55Z`）。NAR / Ban-ei は同一のアーキテクチャ fix が適用されているが、本セッションでは fix 後の clean smoke test で個別に再確認できていない（fix 前の instrumentation では NAR も同じ ~15-17 分 timeout パターンで完走していなかったことを確認済みなので、根本原因は共通と推定される）。同日中に NAR の独立確認を追加で試みたが、同カテゴリ内の同時実行 slot 競合（後述）により test message が dead-letter され、NAR 単独の genuine 書き込みは本ドキュメント時点でも未確認のままである。さらに別セッションで低競合な単発 NAR smoke test を実施し、今度は同時実行競合なしで 10/10 layer を完走させたが（§5.9 item 17）、layer 完了後の scoring/Neon UPSERT ステップの書き込みは time budget 内に確認できず、NAR は依然として未確認のままである。詳細・残存リスクは §5.9 を参照。
...
- **相関/interaction 由来の新規 derived feature 3 候補（2026-07-02、ユーザー指示によるセッション）** — 「新規カラム raw 追加ではなく相関/interaction 分析から真に新しい特徴量を作る」という方針で 3 候補を設計・検証。いずれも既存の pacestyle layer（`rs_p_*` x field pace pressure）、within-race rank/diff 特徴、`weight_zscore` とは機構的に別物であることを事前確認した上で着手（restatement ではない）。全カテゴリで本番 CatBoost/XGBoost の verbatim hyperparameters（`model_meta.json` 参照 model_version の `metadata.json` から読み込み）、3-fold WF blind（train≤2022/2023/2024→blind 2023/2024/2025）、bootstrap LB95（2000 iterations）、§7.2 accept gate で評価。JRA は `tmp/candidate-eval-jra/base-store`（254/263 = 96.6% 本番特徴量一致）、Ban-ei は `tmp/candidate-eval-banei/store-full`（130/130 = 100% 一致）、NAR は on-disk に 192 特徴量 full-parity store が存在しないため conservative reduced baseline（117/192 = 61%、`tmp/candidate-eval-nar/feature_lists.json` の `baseline` を踏襲）を使用——同セッション内の別 lever（career-rate rank-in-race 等）と同じ精度上の理由による妥協であり、NAR の REJECT はこの縮小 baseline でも full 192 特徴量契約に対して conclusive（baseline を増やすほど incremental value は下がる方向のみ）、ADOPT の場合のみ追加の full-pipeline regen 確認が必要という既存 methodology を踏襲。
  - **jockey-track chemistry（`jockey_keibajo_chemistry` / `jockey_distance_chemistry` / `jockey_track_chemistry` / `jockey_grade_chemistry` = 各 jockey subgroup 勝率 − jockey career 勝率、raw rate の絶対値ではなく career baseline からの相対差分）REJECT（JRA/Ban-ei は conclusive、NAR は未完了）**。JRA（pooled n=10,365、2023-2025）: top1 Δ-0.27pp（LB95 -0.62）、place2 Δ+0.03pp（LB95 -0.41）、place3 Δ+0.26pp（LB95 -0.19）、place4〜6 も LB95<=0.05、gate `gate_pos_top3_lb95=0` で REJECT、cell-level pocket 0 件。Ban-ei（pooled n=5,232）: top1 Δ-0.15pp（LB95 -0.54）、place2 Δ+0.11pp（LB95 -0.42）、place3 Δ+0.29pp（LB95 -0.29）、pocket 0 件、REJECT。**NAR は候補ディレクトリのみ作成されセッション終了までに学習が着手されず未完了**——ADOPT/REJECT いずれの結論も出せていない。JRA/Ban-ei の結果パターン（絶対値では小さいが place3 方向に一貫して正、しかし LB95 は全て 0 をまたぐ）から、NAR で結果が反転する具体的な仮説的根拠はない。DO-NOT-RETEST は JRA/Ban-ei のみに適用。NAR は次回セッションで同一 harness（`tmp/candidate2-jra` 系の `eval_jockey_chemistry.py` パターンを NAR 向けに再利用可）で完了させること。
  ...
  - **pace-style × distance-band structural fit score（`pace_style_distance_fit_score` / `pace_style_distance_fit_place_score` = 馬自身の `rs_p_nige/senkou/sashi/oikomi` と、fold の train partition のみから再計算した distance_band×style 別 historical win/place rate の内積、leak-free 設計）— INCOMPLETE / 未確定、本番無変更**。JRA は 2025 blind fold のみ完了（2023/2024 fold は未実行のままセッション終了）。2025-only（n=3,455）: top1 Δ+0.23pp（LB95 -0.41）、place2 Δ+0.12pp（LB95 -0.58）、place3 Δ+0.55pp（LB95 -0.20）——primary 3 指標とも LB95 は 0 をまたぎ、この 1 fold だけでは §7.2 gate を判定できない（supplementary な place4/place5 のみ LB95 わずかに正 +0.12pp/+0.03pp）。`rs_p_*` は 2022/2023 年で ~100% NULL、2024/2025 年でようやく populated（正確な NULL 率は `rs_p_nige_null_pct_by_year` 参照）のため、3-fold pooled にすると希薄化がさらに強まる方向にのみ働くと予想される。NAR/Ban-ei は着手されず。**ADOPT/REJECT いずれの結論も出せておらず、DO-NOT-RETEST リストには加えない**——次回セッションでの完了候補として、(1) JRA の 2023/2024 fold 実行、(2) NAR での同一設計の実行、(3) serve 時点の point-in-time-correct lookup table 更新機構の設計、が残タスクとして必要。
