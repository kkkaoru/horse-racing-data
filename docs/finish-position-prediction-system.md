...
`train-cells` の LightGBM resource control は `--num-threads auto` が既定である。auto は macOS の load average、空きメモリ、compressor 使用量から fit ごとの thread 数を決め、さらに `/tmp` の slot lock で同時 fit 数を制御する。明示的な固定値が必要な検証時だけ `--num-threads 1` のように指定する。
