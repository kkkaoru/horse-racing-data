...
- **NAR**: feature / 学習窓 / 単一アーキ切替（CatBoost）/ venue routing の各 lever はいずれも REJECT で、これらの軸では iter12 XGBoost が frontier。ただし **アーキテクチャ lever（listwise Set Transformer × XGBoost blend）は本物の deployable win（2026-07-02 深夜に gate 通過、2026-07-03 に `iter40-nar-settransformer-blend-v1` を本番 image へ deploy → Neon-write smoke CLEAN → Cloudflare production path で ENABLED → 初回 genuine 本番 write 確認、本キャンペーン初のアーキテクチャ lever win）** であり、この 1 点で従来の「NAR frontier 確定」を更新する。現行 NAR serving = iter40 transformer blend（2026-07-03 production LIVE 確認済み）。
  - **listwise Set Transformer × XGBoost blend = genuine deployable win（gate 通過 2026-07-02 深夜、prod-base gate + serve-exact gate 通過・2026-07-03 Cloudflare production ENABLED = iter40、CF env=1、本キャンペーン初のアーキテクチャ lever win）** — 過去の全アンサンブル campaign（`project_ensemble_campaign_complete_2026_06_18`、5-for-5 ABORT）に反し、**単一アーキテクチャの swap ではなくレース内 cross-horse attention を持つ listwise Set Transformer を XGBoost に blend する**という新しい lever が NAR で初めて accept gate を通過した。
    ...
    - **score-level z-fusion が deployed rank-fusion を 5-fold で上回る（2026-07-03、CONFIRMED ＝実装 + smoke 進行中・deploy は smoke 確認後）** — deploy 済み iter40 は within-race **rank-level** の 0.5 / 0.5 fusion（各馬の within-race rank を平均）。これに対し **score-level z-fusion**（base score と transformer seed-score をそれぞれ race 内で z-正規化してから 0.5 / 0.5 で混ぜる ＝ rank が捨てる confidence magnitude を保持する）を、既存 WF fold predictions を再利用し**再学習ゼロ**で検証した（serve-exact な ketto tie-break で eval==serve を保証）。当初 3-fold では place3 の pooled LB95 が境界（-0.02）だったため **HOLD** としていたが、**2021 / 2022 の blind fold を追加学習して 5-fold pooled（66,883 races）で再検証した結果 CONFIRMED**。**決定 ＝ flip 実装中（本番 iter40 は smoke + flip 完了まで無変更）**。artifact: `apps/pc-keiba-viewer/tmp/candidate-score-fusion-confirm/`（5-fold fusion tables / 2021-2022 base+transformer preds / `eval_confirm.py`）。
      ...
      - **flip コストは fuse 関数 1 箇所のみ**: `transformer_scorer.py::fuse_ensemble_transformer` を rank→score-z に変更（`within_race_zscore` helper 追加、推奨 variant = `score_z_55`）。model / norm.json / artifact / Dockerfile / 再学習すべて無変更で、既存 flag の裏で完結する。
      - **scope caveat**: 5-fold confirm は harness の iter12 base に対してであり、実 serving は per-class ensemble（iter12 + iter30 + iter36）。z-norm はレース内スケール不変なので移植可能だが、**実 ensemble base での plumbing smoke を経てから flip する**（実装 + smoke 進行中、deploy は smoke 確認後）。deployed iter40（rank-fusion）は smoke + flip 完了まで無変更。
