...
| NAR      | `iter40-nar-settransformer-blend-v1` | XGBoost ensemble × Set Transformer rank-fusion | 192（base）＋117（transformer） | full（2006+） | rank:pairwise ＋ listnet（0.5/0.5 fusion） |
| Ban-ei   | `banei-cb-v9-sim-2011`               | CatBoost                                       | 130                             | 2011+         | YetiRank                                   |

> **NAR iter40 transformer blend = production-wide ENABLED（2026-07-03）**: NAR は 2026-07-03 に iter40 transformer blend（`iter40-nar-settransformer-blend-v1`）を **両本番 write path で有効化済み**。表の NAR 行は iter12 ensemble（192 feat XGBoost）を base に、117-feat listwise Set Transformer を within-race rank-fusion（0.5/0.5）した現行 serving 構成を指す（fallback は iter12）。有効化方式は 2 path: (1) **Mac launchd path**——`scripts/launchd/finish-position-predict-daily.sh` が docker run に `NAR_TRANSFORMER_BLEND_ENABLED`（default 1）を渡す（commit `2e6c8c4c`、次回 launchd run で有効）、(2) **CF worker path**——`container-class.ts` が env を container へ forward（commit `28c89abf`）＋ wrangler **secret** `NAR_TRANSFORMER_BLEND_ENABLED=1`（redeploy で消えない）＋ worker deploy（Version `0d2d99c7-cbe1-4989-a497-255e73e77388`、次回 per-race container start で有効）。Neon-write smoke は CLEAN（iter40 行が本番 Neon に landing・fusion が iter12 と異なる・fallback 確認済み）。rollback = env/secret を 0 に（redeploy 不要）。**enable は済んだが、初回の実本番 NAR write（Mac 03:00 JST or CF per-race）での iter40 landing は 2026-07-03 に最終確認予定**。詳細・検証・rollback は §2.5 / §5.9、統制・機構は §11。
...
### 2.5 NAR Set-Transformer blend（2026-07-03 production-wide ENABLED・iter40）

NAR の iter40 transformer blend（`iter40-nar-settransformer-blend-v1`）は 2026-07-03 に **両本番 write path で production-wide ENABLED**——**NAR serving は iter40 transformer blend に切替済み**（§2 本番モデル表の NAR 行がこれを指す）。有効化は Neon-write smoke が CLEAN で完了（iter40 行が本番 Neon に landing・fusion が iter12 と異なる place-refining・fallback → iter12 を direct prod-Neon query で確認）したことを受けて実施した。有効化方式は 2 path: **Mac launchd** = `scripts/launchd/finish-position-predict-daily.sh` が docker run に `NAR_TRANSFORMER_BLEND_ENABLED`（default 1）を渡す（commit `2e6c8c4c`）／**CF worker** = `container-class.ts` が env を container へ forward（commit `28c89abf`）＋ wrangler **secret** `NAR_TRANSFORMER_BLEND_ENABLED=1`（redeploy 耐性）＋ worker deploy（Version `0d2d99c7-cbe1-4989-a497-255e73e77388`）。iter40 は serve-exact gate ADOPT 済み。**残るのは初回の実本番 NAR write（Mac 03:00 JST or CF per-race）での iter40 landing の最終確認のみ**（enable は済んだが実 write はまだ観測されていない）。アーキテクチャ lever が accept gate を通過した経緯・統制・機構は §11、deploy 記録・検証・rollback は §5.9 の 2026-07-03 ブロックを参照。

- **手法**: 本番 `iter12-nar-xgb-hpo-v8`（XGBoost・192 feat・per-class ensemble iter30 / iter36 residual を含む serve base）の within-race rank と、117-feat の 3-seed listwise Set Transformer（全 history 学習・レース内 cross-horse self-attention ＋ listnet loss）の within-race rank を **0.5 / 0.5 で rank-fusion**（同順位は `ketto`＝血統登録番号の昇順で tie-break）する。base / fallback は iter12 ensemble。
...
- **flag と有効化（両 path で ENABLED=1）**: `NAR_TRANSFORMER_BLEND_ENABLED`（env override）。**両本番 write path で env=1 有効化済み**——(1) **CF worker**: wrangler **secret** `NAR_TRANSFORMER_BLEND_ENABLED=1` として設定（redeploy で消えない drop-proof、live-rollbackable）、`container-class.ts`（commit `28c89abf`）が env を container へ forward、worker deploy Version `0d2d99c7-cbe1-4989-a497-255e73e77388`（container image 再利用、crons/queue/observability intact）。次回 per-race container start で有効。(2) **Mac launchd**: `scripts/launchd/finish-position-predict-daily.sh`（commit `2e6c8c4c`）が docker run に `NAR_TRANSFORMER_BLEND_ENABLED`（default 1）を渡す。次回 launchd run（03:00 JST nar/ban-ei、09:30 JST all）で有効。**rollback**（redeploy 不要）: CF = `printf 0 | bunx wrangler secret put NAR_TRANSFORMER_BLEND_ENABLED`、Mac = `$REPO_ROOT/.env` に `NAR_TRANSFORMER_BLEND_ENABLED=0`。いずれも即時に pure iter12 ensemble へ戻る。**注意**: base image は default=False を bake している（HEAD からの rebuild + redeploy でも transformer は default OFF）ため、有効化は上記 secret / env=1 の明示で成立している。詳細は §5.9。
...
26. **CLOSED 証跡（同カテゴリ 2 レース連続 re-smoke、同一 `predict-nar` プロセス）**: fix 後に同一 predict-nar プロセスで NAR 2 レースを連続処理させ、両方が clean write することを確認した——`55:05`（1 本目）は `model_version='iter12-nar-xgb-hpo-v8'` の 9 行を `2026-07-02T09:19:05Z`（layer10/10 完了の約 5 秒後）に、**`55:06`（2 本目）は `model_version='iter36-nar-lgb-ensemble-C-v8'` の 12 行を `2026-07-02T09:40:17Z`（layer10/10 完了の約 3 秒後）に**書き込んだ。fix 前は 2 本目が必ず書き込みゼロ（例: item 17 / item 21 の `54:04` は layer を 2 回完走しても 0 行）だったのに対し、retry 不要の単発完走で clean write が成立した。これにより item 17 / item 21 の残タスクは CLOSED、§2 冒頭のとおり **JRA（`10:01`）/ NAR（`54:03` / `55:05` / `55:06`）/ Ban-ei（`83:03`）の 3 カテゴリすべてで genuine per-race 書き込み + 同カテゴリ複数レース逐次処理の正常化を確認済み**。**rollback**: image を `59aaeeba` に戻す（`git revert c3a48694` → `finish-position-cron` deploy）。残存リスク（低）は cold-start transient（初回 `/predict` 失敗、queue retry で回復、warm-up は今後検討）と busy budget のタイトさのみで、budget は `b6f7500e` で 80 分に強化済み。さらに同日 19:47 JST までに、現行 serving image `ec62072a` 上で JRA（`jra:2026:0627:02:01`、16 行、`jra-cb-v9-sim-2013`、prediction_generated_at=10:35:56Z、16/16 layer 完了直後）と Ban-ei（`83:04`、10 行、`banei-cb-v9-sim-2011`、10:16:33Z、7/7 layer）の genuine write も確認し、**3 カテゴリ全てが現行 image 上で per-race serving 検証済み**となった。

**2026-07-03 NAR Set-Transformer blend: production-wide ENABLED（iter40、serve-exact gate ADOPT 済み、Neon-write smoke CLEAN → 両 write path で env=1 有効化）**

§11 で「genuine win だが serve-path parity を閉じる段階（当時 DEPLOY IN PROGRESS）」としていた NAR Set-Transformer × XGBoost rank-fusion blend を、serve-exact な numpy scorer で parity を閉じ本番 image へ deploy し、full 本番 Neon-write smoke を CLEAN で通した上で **両本番 write path（CF worker + Mac launchd）で env=1 有効化済み**。**NAR serving は iter40 transformer blend に切替済み**。ただし enable 済みであって、初回の実本番 NAR write での iter40 landing はまだ観測されていない（過大表現を避けて正確に記す。下記 item 30 参照）。モデル仕様は §2.5、science / 統制 / 機構は §11 を参照。

27. **deploy 済み ＋ 両 path ENABLED**: transformer コード + R2 artifact を bake した本番 image を deploy 済み（base image は flag default=False を bake）。その上で有効化を実施: **CF worker** = wrangler **secret** `NAR_TRANSFORMER_BLEND_ENABLED=1`（redeploy 耐性の drop-proof、live-rollbackable）＋ `container-class.ts` の env forward（commit `28c89abf`）＋ worker deploy Version `0d2d99c7-cbe1-4989-a497-255e73e77388`（container image 再利用、crons / queue / observability（`head_sampling_rate: 0.1`）は intact）。**Mac launchd** = `scripts/launchd/finish-position-predict-daily.sh`（commit `2e6c8c4c`）が docker run に `NAR_TRANSFORMER_BLEND_ENABLED`（default 1）を渡す。**dual-path 整合**: CF worker（secret=1）と Mac launchd（env default=1）の両 serving path が iter40 有効で一致。viewer は latest-for-race（priority-3）で自動的に iter40 行を拾う（下記 item 28）。
28. **viewer 表示**: finish-position query の priority-3（latest-for-race catch-all、`order by priority, recency desc`）機構は unchanged で、iter40 の書き込みが始まれば同じ priority-3 機構が iter40 の行を latest-for-race として自動的に表示する（active_models の flip は不要）。初回の実本番 iter40 write までは、その race について最新である iter12 行が表示される。
29. **検証済み（enable 前の証跡 ＋ Neon-write smoke）**: (a) serve-exact（ketto tie-break）gate ADOPT（top1 +0.629[LB95 +0.467] 他）、(b) deployed module が canonical scorer に byte-exact（max diff 0、順位 flip 0）、(c) wiring smoke——real image で NAR → iter40 routing、`predicted_score` が `fuse_ensemble_transformer` に bit-exact、両 fallback tier（field<2 / feature 欠損）→ iter12 ensemble を確認、(d) **full 本番 Neon-write smoke CLEAN**——env=1 で NAR `55:03` を trigger し、本番 Neon `race_finish_position_model_predictions` に iter40 の 11 行が landing（generated 01:55 JST、55:03Z 頃 write）、うち 5/11 頭が iter12 と reorder（transformer fusion が place を refine している＝ iter12 と別出力であることを確認）、fallback → iter12 も確認。direct prod-Neon query で検証。**これで §2.5 item 30(a) の環境ブロックは解消し、両 write path での有効化に至った**。
30. **残る確認（正直に）**: (a) full 本番 Neon-write smoke は **CLEAN で完了済み**（item 29(d)——env=1 で trigger した NAR `55:03` の iter40 11 行が本番 Neon に landing、5/11 頭が iter12 と reorder、fallback 確認）。当初 local Colima PG replica に `cell_model_key` / `cell_variant_id` 列が無い環境要因で blocked だったが、本番 Neon 経路の smoke で解消。(b) **env=1 有効化済み**（両 write path、item 27）。**残るのは (c) のみ**: **初回の実本番 NAR write（Mac 03:00 JST or CF per-race）での iter40 landing の最終確認**——2026-07-03 の次回スケジュール実行で `race_finish_position_model_predictions` に `model_version` が iter40 の行が landing することを確認する（enable は済んだが、実 write はまだ観測されていない）。(d) 問題あれば env/secret=0 で即 rollback。
31. **rollback と rebuild 注意（現状 ENABLED）**: 現状は両 path で有効（CF secret=1、Mac env default=1）。**無効化 / rollback（redeploy 不要・即時に pure iter12 ensemble へ戻る）**: CF = `printf 0 | bunx wrangler secret put NAR_TRANSFORMER_BLEND_ENABLED`、Mac = `$REPO_ROOT/.env` に `NAR_TRANSFORMER_BLEND_ENABLED=0`。**注意**: base image は transformer default=False を bake しているため、有効化は上記 secret / env=1 の明示に依存する（HEAD からの rebuild + redeploy でも default OFF）。iter40 を image から完全に取り除く場合は該当 deploy を revert + image rebuild して旧 image へ deploy。
...
| `metric_payload`                       | target 固有の詳細 JSONB。`finish_position` / `running_style` とも per-cell accuracy / evaluation payload を保存する。payload 未指定の評価は `cell_training_evaluation_scalar_v1` を自動生成し、running-style の新規 cell 学習は `running_style_cell_v2` として class 別指標、confusion matrix、race-level corner / finish 指標を保存する |
...
- **NAR**: feature / 学習窓 / 単一アーキ切替（CatBoost）/ venue routing の各 lever はいずれも REJECT で、これらの軸では iter12 XGBoost が frontier。ただし **アーキテクチャ lever（listwise Set Transformer × XGBoost blend）は本物の deployable win（2026-07-02 深夜に gate 通過、2026-07-03 に `iter40-nar-settransformer-blend-v1` を本番 image へ deploy → Neon-write smoke CLEAN → 両本番 write path で production-wide ENABLED、本キャンペーン初のアーキテクチャ lever win）** であり、この 1 点で従来の「NAR frontier 確定」を更新する。現行 NAR serving = iter40 transformer blend（初回の実本番 write 確認のみ残）。
  - **listwise Set Transformer × XGBoost blend = genuine deployable win（gate 通過 2026-07-02 深夜、prod-base gate + serve-exact gate 通過・2026-07-03 production-wide ENABLED = iter40、両 write path env=1、本キャンペーン初のアーキテクチャ lever win）** — 過去の全アンサンブル campaign（`project_ensemble_campaign_complete_2026_06_18`、5-for-5 ABORT）に反し、**単一アーキテクチャの swap ではなくレース内 cross-horse attention を持つ listwise Set Transformer を XGBoost に blend する**という新しい lever が NAR で初めて accept gate を通過した。
    ...
    - **deploy 状況（正確に、2026-07-03 production-wide ENABLED = `iter40-nar-settransformer-blend-v1`、両 write path env=1）**: 本番着順予測は CF Container 経由必須（§1 / §9）で、当初 **MLX は Container で動かず、ONNX 変換は torch 非在で不可、numpy forward は MLX 出力と corr 0.998 だが bit-exact でなく、接戦レースで順位 flip の serve-skew リスクがある** ことが deploy blocker だった。これを **pure numpy float64 の serve-exact scorer（`predict_lib/transformer_scorer.py`、MLX-eager に byte-exact ＝順位 flip 0）で解消**し、serve-exact な ketto tie-break gate を再実行して ADOPT を確認（eval==serve を保証）した上で本番 image へ deploy、**full 本番 Neon-write smoke CLEAN 後に両本番 write path で env=1 有効化**した（CF worker = wrangler secret、Mac launchd = env default 1。commit `28c89abf`（CF forward）/ `2e6c8c4c`（Mac）、worker Version `0d2d99c7`。**現行 CF / Mac の NAR serving は iter40 transformer blend**。deploy 記録・検証・rollback は §5.9 の 2026-07-03 ブロック、モデル仕様は §2.5 を参照）。過去の phantom deploy（serve と eval の乖離、§2.3 / `project_rs_calibration_deployed_2026_06_12` の教訓）に従い serve-exact 確認を経ているため、eval==serve が保証される。**残るのは初回の実本番 NAR write（Mac 03:00 JST or CF per-race）での iter40 landing の最終確認のみ**（enable は済んだが実 write はまだ観測されていない）。
    - **artifact**: `apps/pc-keiba-viewer/tmp/candidate-mlx-nar/`（`gate_prodbase.json` / `final_summary.json` / `eval_prodbase.py` / `numpy_forward.py` / `v9_feature_list.json` / `ckpt/c2v9_s*`）。serve-path parity（serve-exact numpy scorer）解決 + Neon-write smoke CLEAN 後に `iter40-nar-settransformer-blend-v1` を本番 image へ deploy し両 write path で env=1 有効化——現行 NAR serving は iter40 transformer blend（2026-07-03 production-wide ENABLED、初回の実本番 write 確認のみ残、§2.5 / §5.9）。
