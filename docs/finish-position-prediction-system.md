...
- **NAR**: feature / 学習窓 / 単一アーキ切替（CatBoost）/ venue routing の各 lever はいずれも REJECT で、これらの軸では iter12 XGBoost が frontier。ただし **アーキテクチャ lever（listwise Set Transformer × XGBoost blend）は本物の deployable win（2026-07-02 深夜、本キャンペーン初のアーキテクチャ lever win、deploy 準備中）** であり、この 1 点で従来の「NAR frontier 確定」を部分的に更新する。
  - **listwise Set Transformer × XGBoost blend = genuine deployable win（2026-07-02 深夜、本キャンペーン初のアーキテクチャ lever win、prod-base gate 通過・DEPLOY IN PROGRESS）** — 過去の全アンサンブル campaign（`project_ensemble_campaign_complete_2026_06_18`、5-for-5 ABORT）に反し、**単一アーキテクチャの swap ではなくレース内 cross-horse attention を持つ listwise Set Transformer を XGBoost に blend する**という新しい lever が NAR で初めて accept gate を通過した。
    ...
    - **deploy 状況（正確に、DEPLOY IN PROGRESS）**: genuine win だが本番反映は未完了。本番着順予測は CF Container 経由必須（§1 / §9）で、**MLX は Container で動かず、ONNX 変換は torch 非在で不可、numpy forward は MLX 出力と corr 0.998 だが bit-exact でなく、接戦レースで順位 flip の serve-skew リスクがある**。残 deploy タスク = **serve-exact な numpy scorer で gate を再実行（eval==serve を保証）＋ per-cell/category-default routing との整合確認 ＋ Container 統合**。「genuine win だが deploy 前に serve-path parity を閉じる」段階として記録する（過去の phantom deploy = serve と eval の乖離、§2.3 / `project_rs_calibration_deployed_2026_06_12` の教訓に従い、serve-exact 確認前は flip しない）。
    - **artifact**: `apps/pc-keiba-viewer/tmp/candidate-mlx-nar/`（`gate_prodbase.json` / `final_summary.json` / `eval_prodbase.py` / `numpy_forward.py` / `v9_feature_list.json` / `ckpt/c2v9_s*`）。本番 `iter12-nar-xgb-hpo-v8` は serve-path parity 解決まで無変更。
