...
- **NAR**: feature / 学習窓 / 単一アーキ切替（CatBoost）/ venue routing の各 lever はいずれも REJECT で、これらの軸では iter12 XGBoost が frontier。ただし **アーキテクチャ lever（listwise Set Transformer × XGBoost blend）は本物の deployable win（2026-07-02 深夜に gate 通過、2026-07-03 に `iter40-nar-settransformer-blend-v1` を本番 image へ deploy → Neon-write smoke CLEAN → Cloudflare production path で ENABLED → 初回 genuine 本番 write 確認、本キャンペーン初のアーキテクチャ lever win）** であり、この 1 点で従来の「NAR frontier 確定」を更新する。現行 NAR serving = iter40 transformer blend（2026-07-03 production LIVE 確認済み、同日 fusion を rank → score-level z-fusion に更新＝ deployed win #2）。**アーキテクチャ lever から 2 つの deployable win が本番稼働**（rank-fusion +0.63pp top1 / score-z +0.25pp top1、下記 score-z ブロック参照）。アーキテクチャ lever の全 knob（architecture variant c1/c2/c3・fusion 方式 rank/score-z・blend weight tw・per-cell routing・seed 数）を検証完了し、**deployed = 3-seed c2 listwise + score-z fusion が最適配置**と確定（seed 数 sweep は下記 seed 数 sweep ブロック参照、3-SEED-SATURATED）。
  ...
  - **NAR 大規模調査（systematic error/residual 分解 + 網羅的 feature discovery + speed-figure 再構築）= 全候補 REJECT、frontier 確定（2026-07-03、DO-NOT-RETEST）** — Phase 1 の系統誤差分解から speed-figure 再構築 gate まで通した本セッション最大規模の NAR campaign。結論は「NAR は既に経験的フロンティアで、speed / time 軸にも exploitable な残余 signal は無い」。
    - **Phase 1 error/residual 分解**: NAR モデルは market を全 cell で支配（overall top1 58.62% vs market favorite 44.43% ＝ **+14.2pp**、place3 90.2% vs 77.58%）、market favorite に top1 で負ける cell は皆無。弱い cell（top1 が低い）は市場も同様に弱い **irreducible-difficulty**（南関東の大場 大井 / 船橋・13+ 頭の大頭数 field）で、model の負けではなく当日確率の壁。calibration は良好。odds 統制 partial-ρ の raw 属性 probe は全て in-model reference（`career_win_rate` の 0.039）以下 ＋ 既 documented-REJECT 領域（body-mass / draw / sex / futan / barei）に収束＝新規 raw signal なし。
    - **構造的発見（speed-figure の dead-feature gap）**: store の `speed_index_avg_5` / `speed_index_best_5` が **100% NULL**——`time_sa`（走破タイム偏差）由来だが `time_sa` が NAR で欠損しているため、**NAR モデルは speed-figure signal を dead（常時 NULL）でしか持たない**。一方 `soha_time`（生の走破タイム）は 97.6% populated で、正規化すれば speed figure を再構築できる余地があった。
    - **speed-figure 再構築 gate = REJECT**: `soha_time` を par（距離×トラック基準）＋ daily-track-variant で正規化した Beyer 式 speed figure **10 特徴**を additive。base XGBoost gate（117 reduced baseline、`iter12-nar-xgb-hpo-v8` verbatim、3-fold WF blind）で top1 Δ+0.021[LB95 -0.192]／place2 Δ-0.028／place3 Δ+0.045＝**0/3 primary で LB95>0**。deployed iter40 への blend-survival でも top1 Δ+0.008（blend 後さらに希釈）。**full-cell 168 cell スキャンで multi-year robust な top1 gain を示す cell はゼロ**（最有力の大頭数帯でも top1 Δ+0.06[LB95 -0.34] と flat、cell-routing 候補なし）。style / form arm も同 harness で REJECT。
    - **機序 + 横断 time/speed 結論**: モデルの finish-history 特徴群（`recent_finish` / career rate / class-relative finish）が既に各馬の速さ・質を encode しており、再構築した speed figure は ~collinear で incremental には非予測的——**finish-history が speed の十分統計量（sufficient statistic）**。**横断テーマとして確定: time / speed は NAR・JRA 両 store に実在する real な dead-feature gap（`speed_index` が両カテゴリで dead）だが accuracy lever ではない**（finish-history が sufficient statistic、speed-figure reconstruction は NAR + JRA とも DO-NOT-RETEST）。本番 NAR = iter40（XGBoost ＋ Set Transformer score-z blend）最適で無変更。artifact: `apps/pc-keiba-viewer/tmp/candidate-nar-largescale/`。
