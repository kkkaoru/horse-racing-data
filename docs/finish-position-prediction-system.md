...
- **launchd / ローカル常駐プロセスは本番構成要素ではない**。手元の scheduler、shell wrapper、Docker process、trainer process を本番 trigger / fallback / ordering dependency にしてはならない。
...
本番の生成責務は Cloudflare 側に閉じる。順序の authority は `sync-realtime-data` と `finish-position-cron` / Container の連携であり、ローカル端末・launchd・手元 cron は本番の trigger / fallback / ordering dependency を持たない。
...
脚質 cell model はローカルで `running_style_lightgbm.py train-cells` により **cell 単位で** 学習・評価・promotion plan 生成を行う。採用された variant は header metadata 込みの flatbin を `RUNNING_STYLE_MODELS` の R2 object として promotion し、`RUNNING_STYLE_CELL_ROUTING_JSON` はその R2 key を指す。production は Cloudflare Worker / Queue / R2 / D1 / Neon のみを参照し、ローカル端末上の model path、launchd job、process に依存しない。
...
3. `trained_cells[*].cell_training_evaluation` は `cell_training_evaluations.prediction_target = 'running_style'` に投入できる互換 mapping を持つ。ただし `train-cells` 単体では Neon へ upsert せず、本書は専用 importer command も定義しない。cell 単位評価を永続化する場合は、実装済みの明示的な upsert 経路を用意してから行う。
...
この guard は Cloudflare Worker / Queue と、fallback 時の service binding の中で完結する。launchd、ローカル cron、手元 shell script、ローカル Docker process で feature → running-style → finish-position の順序を補完・代替してはならない。
...
- **Container DO（`FinishPositionPredictContainer`）**: Python HTTP server を内包し、DuckDB ビルドと scoring を実行する。`instance_type: standard-4`, `max_instances: 10`。
...
上記以外の順序は本番仕様ではない。finish-position full generation を feature generation より前、または running-style generation 完了前に走らせる fallback は作らない。launchd / ローカル wrapper はこの順序の authority ではなく、障害時の再実行境界も Cloudflare Queue retry / DLQ と Worker state に置く。
...
- production は Cloudflare-only である。launchd / ローカル Docker / Python / trainer process は学習・検証・artifact 生成・手元再現用であり、本番の trigger、ordering、retry、fallback、model serving の依存先にしてはならない。local/manual の immediate fallback は、ローカル PostgreSQL を source として Neon へ書く明示的な再同期・backfill に限り、本番 ordering の代替にしない。
...
flowchart TB
    ...
    I["launchd 依存の本番運用<br/>→ CF Queue / Cron / Worker"]
...
9. **launchd 依存の本番運用禁止** — launchd、ローカル cron、手元 shell script を本番 trigger / ordering / retry / fallback に使わない。本番順序は Cloudflare Cron / Queue / Worker / Container で担保し、service binding / API は queue primary path が使えない環境の fallback に限る。
