...
### 2.1.2 JRA Stage-1 市場非依存 gated fallback（base path LIVE、2026-08-24 更新は staged local）
...
**Stage-1（floor）/ Stage-2（既定・champion）の gated 二段構成**を 2026-07-22 に本番実装した。定常時（odds fresh）は Stage-2 = champion がそのまま serve し、精度は無変更。odds-serving incident 時のみ Stage-1 へ切り替わる。2026-08-24 の staged-local 更新では Stage-1 を `jra-cb-stage1-marketfree235-iter500-top1swap-2013` にした。これは従来 base `jra-cb-stage1-marketfree235-2013`（235 feat = champion の 250 feat から上記 15 列を除いた集合、CatBoost YetiRank・iterations 300・lr 0.05・depth 8・l2 3.0・relevance 3/2/1・no_cat_features・seed 20260519・2013-01-01〜2025-12-31 full single-fit、626,798 rows / 44,907 races — champion と同一 population）と、同一特徴量・同一母集団で iterations だけ 500 にした companion の双方をレース単位で score する。companion の rank-1 と base の rank-1 の raw score だけを交換し、base の rank-2 以下を完全に保存する。blind WF 10,365 races で Top1 28.8857%→29.2812%（+0.3956pp、LB95 +0.0772pp、2023/2024/2025 全 fold 正）、Place2 -0.0386pp、Place3 -0.0193pp、Place4 と Top3 box -0.0096pp、Place5/6 ±0.000pp。追加10-loop（iterations 400/450/550/600、lr 0.04/0.06、l2 1/5、random strength 0.5/2）は現行iter500をbaselineに2023-2024の6,910レースで再比較したが、全候補のpooled Top1点推定が負（最良600 iterationsも -0.0579pp[LB95 -0.4920]、年別符号不一致）だったため全REJECT、2025は未開封のままiter500を維持した。従来 base 単体でも odds-collapse 時の champion 9.44% に対し +19.45pp[LB95 +18.44] recovery、健全時比較コストは −4.75pp だった。更新は local artifact/routing と MLflow model-version 登録までで、registry の champion/challenger alias は変更せず、本節執筆時点では未 deploy。
