...
> **重要な留保・更新（2026-07-02 investigation → 同日中に 3 カテゴリすべてで解決、詳細は §5.9）**: 本欄は当初、「本番の per-race パイプラインが実際にこれらのモデルで着順予測を完走し Neon に書き込んだという確認済みの証跡が、テーブル観測可能な履歴の範囲（2026-05-15 以降、約 1.5 ヶ月）で 3 カテゴリいずれについても存在しない」という critical finding を記録していた。**同日中の root-cause 特定と 3 段階の fix（commit `2d3535be` instrumentation → commit `af1ca40e` queue-consumer hold time 分離 → commit `f4b3ea7a` same-category slot starvation 修正）により、JRA / NAR / Ban-ei の 3 カテゴリすべてで live smoke test で genuine な per-race Neon 書き込みを確認済み**: JRA（`jra:2026:0628:10:01`、`race_finish_position_model_predictions` に `model_version='jra-cb-v9-sim-2013'` の 18 行、`prediction_generated_at=2026-07-02T00:09:55Z`）、NAR（`54:03`、`model_version='iter36-nar-lgb-ensemble-C-v8'`——NAR は per-class routing のため本番でも §2 table の `iter12-nar-xgb-hpo-v8` に固定されずクラス別 model_version で書き込む、layer10/10 完了の約 4 秒後 `2026-07-02T06:27:57Z` に 10 行）、Ban-ei（`83:03`、`model_version='banei-cb-v9-sim-2011'`、layer7/7 完了の約 3 秒後 `2026-07-02T06:05:38Z` に 8 行、史上初確認）。残存リスクは same-category concurrency の busy budget（`MAX_BUSY_REQUEUES=40 × 60s = 40 分`）が cold-start transient 多発時にタイトな点のみ（詳細・フォローアップは §5.9）。
...
- **NAR E-top2: DISABLED（本番で一度も serving されていない構造的 blocker を発見）**。`NAR_ETOP2_ENABLED=False` は operational gate だが、flag を立てても構造的に発火しない——override モデル `cb-nar-2013-v8`（196 feat）が要求する 3 特徴量（`bataiju_x_kyori_log`／`bataiju_z_in_barei_kyori_stratum`／`horse_bataiju_trajectory_deviation`）を本番 `finish_position_features_duckdb.py` が出力しない（iter17 履歴ドキュメントにのみ存在する phantom feature）。したがって 2026-06-19 の 4-class ADOPT（A/B/NEW/other、top1 +0.24pp／place2 +0.19pp、`project_nar_etop2_perclass_routing_2026_06_19`）は本番で一度も serving されていない。base（iter12）・override（`cb-nar-2013-v8`）とも当時から無変更のため offline 検証は stale ではない。復活作業（193-feat 再学習 → per-class gate 再検証 → flag on + deploy）を同日中に別 agent が進行中（結果は本節の後続更新で記録する）。
...
22. **既知の残存リスク（未解決、フォローアップ推奨）**: (a) busy budget（`MAX_BUSY_REQUEUES=40 × 60s = 40 分`）は cold-start transient が多発する状況ではタイト——上記 `54:04` は約 40 分待機してギリギリ slot を獲得した。緩和候補: budget 引き上げ / cold-start warm-up / カテゴリ別同時実行数の増加。(b) 新 image の初回 `/predict` に connection 起因の cold-start transient（初回失敗、retry で回復）が残る。
23. **補足 commit（未 deploy）**: `50a86c78` "feat(finish-position): warm viewer cache after focused per-race full completion"（focused-full ack の 3 経路で viewer cache を warm する）は **本タスク時点で未 deploy** であり、次回の `finish-position-cron` deploy で反映予定である。
