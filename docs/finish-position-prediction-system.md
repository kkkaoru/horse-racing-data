...
脚質 local feature generation / bucket evaluation も固定 thread / concurrency を既定にしない。`generate-running-style-local.ts` は Colima capacity と macOS の load average、available memory、compressor 使用量から DuckDB `--threads`、`--memory-limit`、Phase A chunk concurrency、JRA/NAR category concurrency を実行時に解決する。`evaluate-running-style-bucket-21y.ts` の `--chunk-concurrency` も既定は `auto` で、同じ resource snapshot から chunk concurrency と category concurrency を解決する。明示指定した場合だけ固定値を使う。kernel panic / swap pressure 再発防止のため、手元の空きリソースを見ずに `8 threads` / `4 parallel` のような固定値を標準運用に戻してはならない。
