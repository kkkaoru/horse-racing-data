...
| NAR      | `iter40-nar-settransformer-blend-v1` | XGBoost × Set Transformer rank-fusion | 192 + 117 | full（2006+） | rank:pairwise + listnet |
...
### 2.5 NAR Set-Transformer blend（本番・2026-07-03 deploy・iter40）

NAR 本番着順モデルは 2026-07-03 に `iter12-nar-xgb-hpo-v8`（単体 XGBoost）から **`iter40-nar-settransformer-blend-v1`（rank-fusion blend）** へ更新した。アーキテクチャ lever が accept gate を通過した経緯・統制・機構は §11、deploy 記録・検証・残課題・rollback は §5.9 の 2026-07-03 ブロックを参照。
...
- **flag**: `NAR_TRANSFORMER_BLEND_ENABLED`（env override）。`0` で即時 rollback（redeploy 不要・pure iter12 ensemble に戻る）。**運用不整合の注意**: git HEAD（commit `325bf2f1`）は default=False（安全側）だが、本番 deploy 済み image は pre-fix の default=True を bake しているため本番は ON。**HEAD から rebuild + redeploy すると transformer が default OFF になる**（no-regression だが win を失う）ので、再 build 時は env=1 明示 か default 再設定が必要。詳細は §5.9。
...
**2026-07-03 NAR Set-Transformer blend deploy（iter40）: `iter40-nar-settransformer-blend-v1` を本番反映（+0.63pp top1・flag-gated）**

§11 で「genuine win だが serve-path parity を閉じる段階（当時 DEPLOY IN PROGRESS）」としていた NAR Set-Transformer × XGBoost rank-fusion blend を、serve-exact な numpy scorer で parity を閉じた上で本番 deploy した。モデル仕様は §2.5、science / 統制 / 機構は §11 を参照。

27. **deploy 済み**: CF worker Version `6a4d1fa3-c054-43df-885d-46ef64a8a571`、Container image `finish-position-cron-finishpositionpredictcontainer:6a4d1fa3`、Mac local image `finish-position-predict-local:split2` も rebuild 済み。commit `039f144a`（未 push）。crons / queue / observability（`head_sampling_rate: 0.1`）は intact。
28. **viewer 表示**: finish-position query の priority-3（latest-for-race catch-all、`order by priority, recency desc`）で iter40 が表示される。active_models の flip は不要（現在 iter12 が表示されているのと同一機構）。
29. **検証済み**: (a) serve-exact（ketto tie-break）gate ADOPT、(b) deployed module が canonical scorer に byte-exact（max diff 0、順位 flip 0）、(c) wiring smoke——real image で NAR → iter40 routing、`predicted_score` が `fuse_ensemble_transformer` に bit-exact、両 fallback tier（field<2 / feature 欠損）→ iter12 ensemble を確認。
30. **残課題（正直に）**: full 本番 Neon-write smoke は、local Colima PG replica に `cell_model_key` / `cell_variant_id` 列が無い（本番 Neon には存在、並行 RS cell-routing 作業由来）ため feature layer 6 でブロックされ、本ドキュメント時点では未完。**これは blend の欠陥ではなく環境要因**であり、write path 自体は unchanged で 2026-07-02 に prod-ops 側が検証済み（上記 item 24-26）。**本日 2026-07-03 の初回本番 NAR race の write で end-to-end 確認予定**（iter40 rows が Neon `race_finish_position_model_predictions` に landing することを確認する）。
31. **運用不整合（正直に）**: git HEAD（commit `325bf2f1`・安全側）は flag default=False だが、deployed image `6a4d1fa3` は pre-fix の default=True を bake しているため本番は ON。**HEAD から rebuild + redeploy すると transformer が default OFF になる**（no-regression だが win を失う）。再 build 時は env=1 明示 か default 再設定が必要。
32. **rollback**: 即時 = worker env `NAR_TRANSFORMER_BLEND_ENABLED=0`（redeploy 不要・pure iter12 ensemble に戻る）。完全 = commit `039f144a` を revert + image rebuild + 旧 image `ab1e5ece` へ deploy。
...
- **NAR**: feature / 学習窓 / 単一アーキ切替（CatBoost）/ venue routing の各 lever はいずれも REJECT で、これらの軸では iter12 XGBoost が frontier。ただし **アーキテクチャ lever（listwise Set Transformer × XGBoost blend）は本物の deployable win（2026-07-02 深夜に gate 通過、2026-07-03 DEPLOYED = `iter40-nar-settransformer-blend-v1`、本キャンペーン初のアーキテクチャ lever win）** であり、この 1 点で従来の「NAR frontier 確定」を部分的に更新する。
  - **listwise Set Transformer × XGBoost blend = genuine deployable win（gate 通過 2026-07-02 深夜、prod-base gate + serve-exact gate 通過・2026-07-03 DEPLOYED = iter40、本キャンペーン初のアーキテクチャ lever win）** — 過去の全アンサンブル campaign（`project_ensemble_campaign_complete_2026_06_18`、5-for-5 ABORT）に反し、**単一アーキテクチャの swap ではなくレース内 cross-horse attention を持つ listwise Set Transformer を XGBoost に blend する**という新しい lever が NAR で初めて accept gate を通過した。
    ...
    - **deploy 状況（正確に、2026-07-03 DEPLOYED = `iter40-nar-settransformer-blend-v1`）**: 本番着順予測は CF Container 経由必須（§1 / §9）で、当初 **MLX は Container で動かず、ONNX 変換は torch 非在で不可、numpy forward は MLX 出力と corr 0.998 だが bit-exact でなく、接戦レースで順位 flip の serve-skew リスクがある** ことが deploy blocker だった。これを **pure numpy float64 の serve-exact scorer（`predict_lib/transformer_scorer.py`、MLX-eager に byte-exact ＝順位 flip 0）で解消**し、serve-exact な ketto tie-break gate を再実行して ADOPT を確認（eval==serve を保証）した上で本番 deploy した。deploy 記録・検証・残課題（full 本番 Neon-write smoke の環境ブロック、flag default の運用不整合）・rollback は §5.9 の 2026-07-03 ブロック、モデル仕様は §2.5 を参照。過去の phantom deploy（serve と eval の乖離、§2.3 / `project_rs_calibration_deployed_2026_06_12` の教訓）に従い、serve-exact 確認を経てから flip した。
    - **artifact**: `apps/pc-keiba-viewer/tmp/candidate-mlx-nar/`（`gate_prodbase.json` / `final_summary.json` / `eval_prodbase.py` / `numpy_forward.py` / `v9_feature_list.json` / `ckpt/c2v9_s*`）。本番は serve-path parity（serve-exact numpy scorer）解決後に `iter12-nar-xgb-hpo-v8` から `iter40-nar-settransformer-blend-v1` へ更新済み（2026-07-03、§2.5 / §5.9）。
