...
> **NAR の deployed-but-dormant 注記**: NAR は 2026-07-03 に iter40 transformer blend（`iter40-nar-settransformer-blend-v1`）を本番 image へ deploy 済みだが、flag `NAR_TRANSFORMER_BLEND_ENABLED` が OFF のため **dormant**——現行 CF real-time NAR serving は表のとおり **pure iter12 ensemble（`iter12-nar-xgb-hpo-v8`）のまま**であり、iter40 の transformer コードと R2 artifact は baked だが未活性。iter40 は serve-exact gate ADOPT 済みで、env `NAR_TRANSFORMER_BLEND_ENABLED=1` による opt-in 有効化待ち。詳細・有効化条件は §2.5、deploy 記録は §5.9。
...
### 2.5 NAR Set-Transformer blend（2026-07-03 deployed-but-dormant・iter40・env=1 有効化待ち）

NAR の iter40 transformer blend（`iter40-nar-settransformer-blend-v1`）は 2026-07-03 に本番 image へ deploy 済みだが、flag `NAR_TRANSFORMER_BLEND_ENABLED` が OFF のため **dormant（有効化待ち）** であり、**現行 CF real-time NAR serving は依然 pure iter12 ensemble（`iter12-nar-xgb-hpo-v8`）**である（§2 本番モデル表の NAR 行が iter12 のままなのはこのため）。transformer コードと R2 artifact は image に baked だが未活性。iter40 は serve-exact gate を ADOPT 済みで、後述の有効化条件を満たし次第 env=1 で opt-in する——**「NAR = iter40 に切替済み」ではない**。アーキテクチャ lever が accept gate を通過した経緯・統制・機構は §11、deploy 記録・検証・残課題・rollback は §5.9 の 2026-07-03 ブロックを参照。
...
- **flag と有効化**: `NAR_TRANSFORMER_BLEND_ENABLED`（env override）。**deploy 済み image `6a4d1fa3` は default=False を bake しており**（01:32 build が flag-fix commit `325bf2f1` を取り込んだため）、**git HEAD=False と CF deployed=False が一致（both dormant・整合、運用不整合なし）**。有効化は env=1 の opt-in のみ、無効化は env=0（redeploy 不要・即時に pure iter12 ensemble へ戻る）。**残る有効化条件**: (a) full 本番 Neon-write smoke——local replica のスキーマ差で現状 blocked、本日 2026-07-03 の実 race か past-date CF trigger で確認、(b) clean な window で env=1、(c) 初回 write 確認、(d) 問題あれば env=0。**注意**: HEAD からの rebuild + redeploy でも transformer は default OFF のままなので、有効化時は env=1 を明示すること。なお **Mac local image（`finish-position-predict-local:split2`）も HEAD から rebuild して flag=False に閉じ済み（image `d83b24dafa68`）**であり、CF 本番（`6a4d1fa3`）と Mac local（`d83b24dafa68`）の両 serving path が flag=False で dormant・無回帰。詳細は §5.9。
...
**2026-07-03 NAR Set-Transformer blend: deployed-but-dormant（iter40、serve-exact gate ADOPT 済み、flag OFF で env=1 有効化待ち）**

§11 で「genuine win だが serve-path parity を閉じる段階（当時 DEPLOY IN PROGRESS）」としていた NAR Set-Transformer × XGBoost rank-fusion blend を、serve-exact な numpy scorer で parity を閉じ、本番 image へ **deploy 済み**にした。ただし **flag `NAR_TRANSFORMER_BLEND_ENABLED` は OFF で dormant——現行 CF real-time NAR serving は依然 pure iter12 ensemble** であり、iter40 はまだ本番 serving に効いていない（過大表現を避けて正確に記す）。モデル仕様は §2.5、science / 統制 / 機構は §11 を参照。

27. **deploy 済み（dormant）**: CF worker Version `6a4d1fa3-c054-43df-885d-46ef64a8a571`、Container image `finish-position-cron-finishpositionpredictcontainer:6a4d1fa3`。commit `039f144a`（未 push）。crons / queue / observability（`head_sampling_rate: 0.1`）は intact。**この image は flag default=False を bake している**（01:32 build が flag-fix commit `325bf2f1` を取り込んだため）ので、transformer コード + R2 artifact は焼かれているが **未活性**。有効化は env=1 の opt-in のみ。**flag 整合**: git HEAD=False・CF deployed image `6a4d1fa3`=False・Mac local image `finish-position-predict-local:split2`（HEAD から rebuild 済み、image `d83b24dafa68`）=False の全 serving path が一致（all dormant）で運用不整合なし・無回帰。
28. **viewer 表示**: finish-position query の priority-3（latest-for-race catch-all、`order by priority, recency desc`）機構は unchanged で、**dormant の現在は iter12 の行が表示される**。env=1 で有効化して iter40 の書き込みが始まれば、同じ priority-3 機構で iter40 が表示される（active_models の flip は不要）。
29. **検証済み（deploy-ready 証跡）**: (a) serve-exact（ketto tie-break）gate ADOPT（top1 +0.629[LB95 +0.467] 他）、(b) deployed module が canonical scorer に byte-exact（max diff 0、順位 flip 0）、(c) wiring smoke——real image で（flag ON にした場合の）NAR → iter40 routing、`predicted_score` が `fuse_ensemble_transformer` に bit-exact、両 fallback tier（field<2 / feature 欠損）→ iter12 ensemble を確認。これらは blend が有効化可能な状態にあることの証跡であり、本番稼働そのものではない。
30. **有効化の残条件（正直に）**: (a) full 本番 Neon-write smoke——local Colima PG replica に `cell_model_key` / `cell_variant_id` 列が無い（本番 Neon には存在、並行 RS cell-routing 作業由来）ため feature layer 6 でブロックされ、本ドキュメント時点では未完。**これは blend の欠陥ではなく環境要因**であり、write path 自体は unchanged で 2026-07-02 に prod-ops 側が検証済み（上記 item 24-26）。**本日 2026-07-03 の実 race か past-date CF trigger で end-to-end 確認予定**（iter40 rows が Neon `race_finish_position_model_predictions` に landing することを確認）。(b) clean な window で env=1、(c) 初回 write 確認、(d) 問題あれば env=0。
31. **有効化・無効化と rebuild 注意**: 有効化 = worker env `NAR_TRANSFORMER_BLEND_ENABLED=1`（redeploy 不要）。無効化 / rollback = env=0（redeploy 不要・即時に pure iter12 ensemble へ戻る）。**注意**: 現状すでに OFF（dormant）なので追加の rollback は不要。HEAD からの rebuild + redeploy でも transformer は default OFF のままなので、有効化する場合は env=1 を明示すること。iter40 を完全に取り除く場合は commit `039f144a` を revert + image rebuild + 旧 image `ab1e5ece` へ deploy。
...
- **NAR**: feature / 学習窓 / 単一アーキ切替（CatBoost）/ venue routing の各 lever はいずれも REJECT で、これらの軸では iter12 XGBoost が frontier。ただし **アーキテクチャ lever（listwise Set Transformer × XGBoost blend）は本物の deployable win（2026-07-02 深夜に gate 通過、2026-07-03 に `iter40-nar-settransformer-blend-v1` を本番 image へ deploy 済みだが flag OFF で dormant・env=1 有効化待ち、本キャンペーン初のアーキテクチャ lever win）** であり、この 1 点で従来の「NAR frontier 確定」を部分的に更新する。現行 serving は依然 iter12 ensemble。
  - **listwise Set Transformer × XGBoost blend = genuine deployable win（gate 通過 2026-07-02 深夜、prod-base gate + serve-exact gate 通過・2026-07-03 deployed-but-dormant = iter40、flag OFF で env=1 有効化待ち、本キャンペーン初のアーキテクチャ lever win）** — 過去の全アンサンブル campaign（`project_ensemble_campaign_complete_2026_06_18`、5-for-5 ABORT）に反し、**単一アーキテクチャの swap ではなくレース内 cross-horse attention を持つ listwise Set Transformer を XGBoost に blend する**という新しい lever が NAR で初めて accept gate を通過した。
    ...
    - **deploy 状況（正確に、2026-07-03 deployed-but-dormant = `iter40-nar-settransformer-blend-v1`、flag OFF で env=1 有効化待ち）**: 本番着順予測は CF Container 経由必須（§1 / §9）で、当初 **MLX は Container で動かず、ONNX 変換は torch 非在で不可、numpy forward は MLX 出力と corr 0.998 だが bit-exact でなく、接戦レースで順位 flip の serve-skew リスクがある** ことが deploy blocker だった。これを **pure numpy float64 の serve-exact scorer（`predict_lib/transformer_scorer.py`、MLX-eager に byte-exact ＝順位 flip 0）で解消**し、serve-exact な ketto tie-break gate を再実行して ADOPT を確認（eval==serve を保証）した上で本番 image へ deploy した。**ただし deploy 済み image は flag `NAR_TRANSFORMER_BLEND_ENABLED`=OFF を bake しており dormant——現行 CF real-time NAR serving は依然 pure iter12 ensemble** で、iter40 はまだ serving に効いていない（有効化は env=1 の opt-in のみ、残条件 = full 本番 Neon-write smoke の環境ブロック解消。deploy 記録・検証・有効化条件・rollback は §5.9 の 2026-07-03 ブロック、モデル仕様は §2.5 を参照）。過去の phantom deploy（serve と eval の乖離、§2.3 / `project_rs_calibration_deployed_2026_06_12` の教訓）に従い、serve-exact 確認を経ており、活性化しても eval==serve が保証される。
    - **artifact**: `apps/pc-keiba-viewer/tmp/candidate-mlx-nar/`（`gate_prodbase.json` / `final_summary.json` / `eval_prodbase.py` / `numpy_forward.py` / `v9_feature_list.json` / `ckpt/c2v9_s*`）。serve-path parity（serve-exact numpy scorer）解決後に `iter40-nar-settransformer-blend-v1` を本番 image へ deploy 済みだが flag OFF で dormant——現行 serving は `iter12-nar-xgb-hpo-v8` のまま、env=1 有効化待ち（2026-07-03、§2.5 / §5.9）。
