...
### 2.1.2 JRA Stage-1 市場非依存 gated fallback（base path LIVE）
...
**Stage-1（floor）/ Stage-2（既定・champion）の gated 二段構成**を 2026-07-22 に本番実装した。定常時（odds fresh）は Stage-2 = champion がそのまま serve し、精度は無変更。odds-serving incident 時のみ Stage-1 へ切り替わる。live Stage-1 は `jra-cb-stage1-marketfree235-2013`（235 feat = champion の 250 feat から上記 15 列を除いた集合、CatBoost YetiRank・iterations 300・lr 0.05・depth 8・l2 3.0・relevance 3/2/1・no_cat_features・seed 20260519・2013-01-01〜2025-12-31 full single-fit、626,798 rows / 44,907 races — champion と同一 population）である。2026-08-24 に `jra-cb-stage1-marketfree235-iter500-top1swap-2013` を一時 bake したが、R2 再スコアの base が MLflow の実配信精度を再現せず、本番より改善した同一条件の証拠が成立しなかったため REJECT。predict / race-chain は、旧 Stage-1 を含むことを実イメージから確認した `e45334fa` へ rollback した。従来 base 単体は odds-collapse 時の champion 9.44% に対し +19.45pp[LB95 +18.44] recovery、健全時比較コストは −4.75pp である。
...
1. **Freshness gate（一次、scoring 前）**: `race_has_fresh_odds` がレース内の生 `tansho_ninkijun`（late-binding が odds snapshot からそのまま複製する値。中央値 fallback される `odds_score`/`popularity_score` とは異なり、never median-substituted）を確認する。レース内の全頭が「実際の 1-based 順位（`> 0`）」を持たない場合のみ gate fail（odds board 丸ごと欠落）——一部の馬だけ欠落（例: 直前除外）では trip しない。**`> 0` 判定は必須**: 生 JVD odds 列は未確定を非 NULL の `'00'` placeholder で表現し（`reference_jvd_placeholder_semantics` と同型)、上流 SQL の一部経路は cast 前に `'00'` を除去しない。`coerce_optional_int` が `"00"` を int `0` に変換するため、`is not None` だけの判定では偽陽性になる（NAR 実データ 2,730,085 行で検証済み: `tansho_ninkijun` は 0/負値をゼロ件しか持たない）。
2. **Stddev safety net（二次、scoring 後、`enable_stddev_safety_net` が true の category のみ）**: `is_score_spread_degraded` が Stage-2 で既に計算済みの within-race `predicted_score` の母集団標準偏差（`statistics.pstdev`、ddof=0、`serve_health_check.py` の Cluster-B signature と同じ計算）を `stage1_routing.json` の `stddev_threshold` と比較する。odds board 自体は populated に見えるが ranking が別要因で collapse したケースを捕捉する。JRA の**しきい値は 0.4**（`serve_health_check.py` の提案値 0.3 から実データで再検証の上で引き上げ）: 2026-07-12 Cluster B（36/36 レース、stddev 0.048-0.160）、2026-07-18 odds-freeze（36 レース、stddev 0.114-0.354 — `COORDINATOR_ENABLED=0` による staleness は「欠落」でなく「連続的な劣化」勾配になるため、元の 0.3 では該当日の degraded レースの 7/36（~19%）を見逃していた）、2026-07-19 健全日（36 レース、stddev 0.749-1.870）、2026-07-11（別の混在インシデント日、stddev 最小 0.044）の実 Neon prediction データで検証済み。**一部の category はこの信号自体が使えない**（例: within-race z-normalized な score-level fusion——NAR の XGBoost base × Set Transformer blend——は構造的に stddev が collapse しない、548 レース実データで確認済み、stddev 最小 ~0.36）ため、そのような category は `enable_stddev_safety_net: false` を明示し、sentinel しきい値のような magic number ではなく freshness gate 単独でカバーする。

いずれかの条件を満たせば Stage-1 へ route する（両方 fresh かつ non-degraded の場合のみ Stage-2 を維持）。`stage1_routing.json` は category 別の tunable config（`running_style_cell_routing.json` と同じ「config file, not a hardcoded constant」の規約）で、`model_version` と optional `top1_swap_base_model_version` を持つ。後者があるときは両 artifact の feature 順序一致も必須。対象 category が無い場合（今のところ NAR/Ban-ei は未設定）は gate が一切発火せず既存動作のまま。Stage-1 artifact のロードは他の companion artifact（NAR transformer、E-top2）と同じ fail-closed パターン（どちらかの load 失敗・allowlist 未登録・leak 列混入・feature 順序不一致は Stage-1 を今回の run に限り無効化し、champion のみで serve を継続）。`model_meta.PRODUCTION_MODEL_VERSION_ALLOWLIST` は base と top1 companion の双方を許可し、artifact 登録は `production-artifacts.json` / `predict_lib.artifact_integrity` の既存 manifest 規約に従う（§2.1.1）。Cloudflare shadow scorer も同じ 2 artifact / per-race score swap を実装し、Python serve path と順位保存規約を一致させる。
...
### 3.4 Prophet entity trend（JRA / NAR / Ban-ei、cell動的policy）

Prophet は本番 Container でfit・predictしない。`apps/timesfm-finish-position/scripts/build_prophet_production_lookup.py`が対象年より前の履歴だけを使い、開催場・騎手・調教師の月次performance trendを年内全日について事前計算する。生成した小型ParquetはContainer imageの`/app/lookups`へbakeする。

`add-prophet-entity-trend-features.py`は全categoryの`DAY_CHAIN`末尾で、当日出走表からentity codeだけを取得し、事前計算値をDuckDB joinする。したがって重いProphet依存・model load・fitは日次Containerにもper-race pathにも存在せず、per-race scoringの追加処理は出走頭数に比例する標準化とscore加算だけである。既存boosterの入力列は変更せず、最終scoreへの後段調整として実装する。

`prophet_cell_policy.json`は、本番`cell_routing.json`の全variant、最終served model override、および最大粒度のserved signatureごとにON/OFFとweightを保持する。signatureは`resolved cell + routing mode + Stage-2のswap/keep・Transformer利用/fallback結果 + Stage-2 model + Stage-1 gate理由/適用結果 + final model/composite version`で構成する。runtime precedenceはexact signature → final served branch → cell → default ON `0.05`であり、未観測の細分岐でも従来のv4 branch/cell policyへ安全にfallbackする。weight探索は2024・2025と利用可能なpartial 2026の実production feature frameを現在の出荷model graphで再scoreし、score交差点event sweepでTop1、同率ならTop2からTop5を辞書順に最大化する。目的関数は非単調なため、固定grid、linear/binary/ternary search、cell間Cartesian探索は使わない。線形ensemble内部memberは後段補正では個別係数を識別できないため、最終served compositeを1分岐として扱う。`PROPHET_SCORE_ADJUSTMENT_ENABLED`はcategory emergency overrideで、`off`は全停止する。本番では`PROPHET_SCORE_ADJUSTMENT_WEIGHT`を未設定にし、signature/branch/cell固有weightを上書きしない。lookup coverageが半数未満、2頭未満、非有限値、分散ゼロ、設定不正のいずれかでは無変更へfail closedする。v5評価authorityは`apps/timesfm-finish-position/results/prophet-maximum-branch-weight-optimization-2024-2026.json`で、121 maximum branches・39,525 racesを対象とする。2026は通年ではないpartial ledgerとして扱う。
...
**再生成した feature parquet の parity 比較（2026-08-15 追記）**: 上記の「同一ファイル再利用」と異なり、同じ入力から DuckDB feature parquet を別々に再生成すると、並列 `sum` / `avg` の reduction 順により浮動小数点列が machine-epsilon 程度ずれることがある。JRA 18 頭を同一 command/input・`threads=8` で即時再生成した実測では、`jockey_corner_1_norm_avg`、`jockey_recent_corner_1_norm_avg_90d`、`trainer_corner_1_norm_avg` に最大 `1.89e-15` の差が出た。別 arm 間では window `avg` 由来の `jockey_recent_win_rate_diff_from_race_avg` にも `1.39e-17` の差が出たが、`threads=1` では base 198/198 列が完全一致した。このため、**再生成 parquet の byte/value parity を要求する検証は `threads=1` で行うか、浮動小数点列だけ事前宣言した epsilon（通常は absolute `1e-12` 以下）で比較し、整数・文字列・NULL・行集合・列集合は exact のまま比較する**。差を見た後に対象列や epsilon を広げてはならない。`threads=1` を本番運用へ強制する必要はない。現行 CatBoost 250-feature vector で上記4列だけ両 arm の値へ差し替えた実測では、raw score、predicted rank、監査用 softmax 値はいずれも18/18で完全一致し、最大 raw-score 差は `0.0` だった。
...
2026-07-18 時点：この書込済み cache を実際に読む経路を追加した。単一レース scope（coordinator の per-race rescore message）に限り、`_fetch_watermarked_per_race_cache`（`predict_upcoming.py`）が候補 object を一時ディレクトリへ取得し、`pipeline_runner.day_base_covers_entry_list` をそのまま再利用した watermark 検証（現在の `jvd_se`/`nvd_se` 全 entrant が候補 parquet の該当レース行に含まれるか）を通過した場合のみ `final_dir` へ反映する。この検証は entry-list drift（直前の出走取消・追加）と、万一 write 側が別レースの内容を書いてしまった場合（sweepC が発見した種類の破損）の両方を同時に弾く（候補が別レースの行しか持たなければ現在の entrant を一件も cover できず、そのまま reject される）。取得・検証のいずれかが失敗（R2 miss・watermark reject・例外）した場合は例外なく既存の `CacheMissError` にフォールバックし、raw Catalog からの full rebuild を実行する（fail-closed、accuracy への影響なし）。

coordinator の "weight rebuild" trigger（`race-coordinator.ts` の `triggerWeightRebuildIfNeeded`）はもともと single-race scope なしの category 全域 `mode=rescore` message を1本送っており、watermark 対象を単一レースの entry-list に限定できないため無条件で `CacheMissError`→category 全体 full rebuild にフォールバックしていた（このコーディネータが引き起こしうる最大のコスト）。2026-07-18 の fast-follow でこれも修理済み：`realtime_race_sources` から当日の該当 category レースを列挙し、per-race rescore message を1レース1本ずつ送るよう変更（半時間1回の dedup gate 自体は維持、fan-out 全体の発火可否を1回の claim で決める点は不変）。各 message は `keibajoCode`/`raceBango` を持つため `queue-consumer.ts` の `isPerRaceRescore` 判定で自動的に per-race dispatch path（近post coordinator rescore と同一経路）に乗り、上記の per-race watermark cache の対象になる。category 全域 rescore という code path 自体が構造的に消滅した。
...
43. **focused-full completion guard に `force` bypass を追加（実装済み、未 deploy）**: `isFocusedFullPredictionComplete()`（`focused-full-completion.ts`）は `race_finish_position_model_predictions` の行数（`count(distinct ketto_toroku_bango)` が期待頭数と一致するか）のみを見る count-only チェックであり、行の中身（`predicted_score` の妥当性）は一切見ない（この limitation は `focused-full-completion.test.ts` の "documented limitation" test で明示的に characterize 済み）。2026-07-12 の degenerate-score バースト（§1.2 の 07-17 serving incident、Cluster B）のように、行数は揃っているが値が壊れている（within-race score 標準偏差が健全時の約 1/10）レースは、この guard 単体では「完了済み」と誤判定され続け、同一 model_version での再 serve による garbage 行の in-place UPSERT 上書き（primary key が `model_version × horse × race` のため、再 serve すれば viewer 側の変更なしに健全値へ置き換わる）を queue consumer が自動でブロックしてしまう。対処として、`PredictQueueMessage.force`（既存フィールド、これまでは old-date guard 専用——`processMessage` の `isOldDateRunYmd` 分岐のみが参照）を `queue-consumer.ts` の `ackIfFocusedFullAlreadyComplete()` でも参照するよう拡張し、`force === true` かつ focused-full skipDedup メッセージ（`isFocusedSkipDedupMessage`）の場合は `isFocusedFullPredictionComplete()` を呼ぶことなく即座に `false`（＝未完了扱い）を返して通常の full-build パイプラインへ進める。`/api/trigger`（`mode=full&skipDedup=true&keibajoCode=..&raceBango=..&force=true`）が呼び出し経路——`/api/admin/run-focused-full-race` は元々この completion guard を通らない（Container DO を直接 fetch する別経路）ため対象外。テストは `queue-consumer.test.ts` に追加（`isFocusedFullPredictionCompleteMock` が呼ばれないこと・Container `/predict` へ実際に到達することを assert）。**Cluster B 72 レース（07-11/07-12 函館優先）の repair 運用は本 deploy 後に着手、開催時間帯（09:00-17:00 JST）は完全退避**——詳細は `jra-serving-audit-jun-jul-2026-07-17.md` §12 系列の追記および campaign summary の Defect B 項を参照。
...
- **champion freshness retrain（2013→2026-07-12、同一250-feat spec、650,045行、wave6）— gate (a)(c)(d) PASS、gate (b) は2026 blind母集団の構造的不在で未決着（2026-07-17）** — 現行champion（`jra-cb-v9-sim-2013-clean`）は2026 H1（~6ヶ月、当季夏開催含む）未学習+時間減衰重みで当季データが最大重みという構造上の理由から、spec不変のデータ最新化を実施（窓ablationとは別物、DO-NOT-RETEST非抵触）。gate (a) パイプラインfold再現一致・(c) feature-importance安定・(d) spec/leak完全一致は全てPASS。**gate (b) 264-race replay parityのみ未決着**——replay母集団が新モデルの学習窓に完全内包されるためin-sample優位性で交絡し、blind読みにならない（point推定7指標中5件が僅かに負、全てCIゼロ跨ぎ）——unilateralに決着させず透明に報告。**判断: 本日はflipせずMLflow challenger（`jra-finish-position` v34、champion aliasはv13のまま不変）として登録のみ**。週末07-18/19をblind gate化（interim safety check→blind≥200 racesで§8.12 gate→flip判断）——次セッションへの引き継ぎ。doc `jra-champion-fresh2026h1-retrain-2026-07-17.md`（commit `735c783c`、MLflow run `949cd46ccf7c40ce90398bf57008d5f1`）。
- **NAR training-data pooling REJECT（2026-07-17）** — NAR行をJRA armB-250へgroup_weight 1.0/0.3で補助混入も0/3 primary・place3全構成負・no-regression floor割れ、fold-2025 blind confirm省略（phase-1で明確REJECT）。本番無変更。commit `f90205f0`。
- **JRA CatBoost HPO広域探索REJECT（2026-07-17、DO-NOT-RETEST）** — 40 trial Optuna TPE（loss/grow_policy/depth/lr/l2/border_count/random_strength/bagging_temperature）、fold-B/2024勝者がfold-A/2023で10/10非陽性・fold-C/2025 blind confirmもgate不合格——iter20のHPO選択バイアスをclean specでより広い探索空間でも再確認。広域CatBoostチューニングをレバーとして完全クローズ。commit `b292b770`、MLflow `2882e32e`。
- **monotonic constraints REJECT（2026-07-17、DO-NOT-RETEST）** — JRA champion odds/popularity列へのCatBoost native monotone_constraints、narrow（2列）/full（8列、重複列迂回封鎖）とも0/3 primary（narrowのplace3陽性はfold-2024限定で夏4場restrictedでは消失）。commit `21bfe635`。
- **fold-2024異常値の機序解明（2026-07-17）** — NAR-pooling・monotonic・HPOの3件が独立にfold-2024だけ有意信号を示した理由を調査、noise floor拡大やdata品質incidentは棄却、実在するvenue構成異常（京都シェア約2倍・阪神約1/3、2023-2024境界の京都改修復帰と整合）と特定。**評価規約への注意**: fold-2024単独の信号はvenue別cell scanでの裏取りなしに信用しないこと。3件のREJECT判定自体は変更なし。commit `6151607c`。
- **going-nowcast両面クローズ（2026-07-17、DO-NOT-RETEST）** — memory index「生存=going-nowcastのみ」の実体を一次資料で確認、flagship仮説（FPが未走レースに0.0=良placeholderを使う）は既に同doc内で反証済みと判明。team-lead提案の別構成（同日先行レース走破時計vsコース基準時計乖離nowcast×closer_tendency交互作用）を新規probe——JRA-wide/夏4場restricted両方符号不安定+bar未達でREJECT。doc `going-nowcast-assessment-2026-07-17.md`、commit `f5e212dd`。
- **クッション値・含水率（USER決定⑤）additive feature REJECT（2026-07-18、DO-NOT-RETEST）** — 夏4場 historical backfill（70 PDF、1,580行、2018-2026）で2 arm（moisture単独／+cushion_value）を3seed×3fold WFへ投入、probe段階の実相関にもかかわらずpooled（0-1/3 primary）・夏4場restricted 100%被覆（両arm0/3）・全3 blind fold・22 cell全レベルでREJECT。v2唯一の名目pass（place3）はfold-2024単独drivenで本doc既知のfold-2024 anomalyと整合し不採用、serve統合せず。doc `jra-cushion-moisture-pilot-2026-07-18.md`§5、commit `8e55d0a3`。
- **jvd_br（生産者）probe REJECT（2026-07-17、DO-NOT-RETEST）** — EB-shrunk breeder win/top3 rate（10年strictly-causal lookback）、odds+sim_owner_win_rate統制でsign-stableだが0.02 bar未達（max 0.0156/0.0122）——connections家族の市場価格化パターンを再確認。lever bank全4項目消化完了。commit `e7410c22`。
- **viewer dedup監査（2026-07-17）** — `getFinishPositionLambdarankPredictions`のbackfill-row罠を監査、バグなしと構造証明（UPSERT primary keyで同一model_version複数行選択が構造的に不可能、tier-3のcross-model_version比較は全履歴0/65 race×variantでoff-label variant選択実績ゼロ）。commit `8f385592`。
- **feature_guard×self-heal無限retrigger監査（2026-07-17）** — `MAX_SELF_HEAL_ENQUEUES_PER_RACE=2`で既に防御済みと確認（既存testが特性化済み）、D1全履歴でescalatedイベント実績ゼロ。未対応の残存課題: feature_guard skip vs genuine gapの判別シグナル欠如（記録のみ）。commit `8ff54abb`。
- **早期presence guard実装（2026-07-17）** — per-race予測requestでtarget raceがcatalogに0行の場合、17-layer DuckDB build（最大約27.5分）を丸ごとskip。精度影響ゼロ（既存 `if not built: return {}` パスへ早く到達するのみ）。3 test追加、1300 passed、cov99.81%維持。commit `5c3898d0`。
- **serving latency architecture評価（2026-07-17、実装なし）** — `DAY_BASE_SPLIT_ENABLED`は現状flipしても効果ゼロと判明（`e6111ca6`のcatalog-source trust boundaryで`ensure_day_base()`がr2-catalog://に対し常にNoneを返す設計欠陥）。slot serializationは正当な整合性ガード。real headroomはDO-level sharding（次サイクル設計課題）。model booster reload cost実測（初回~~700ms、以降~~9ms、対策不要）。commit `9da2f5eb`。
