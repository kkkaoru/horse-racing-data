...
できたこと: 朝の 80/80 を **host** で埋めた。体重 trigger が発火することを
測った。何が原因**でない**かをいくつか切った。明朝の手順を書いた。
できなかったこと: 発走前に体重を載せること。原因を特定すること。
本番 container で 80 を出すこと。9 件の commit で直すこと。

## 1. 今日の運用

予測は守れた。体重反映は届かなかった。馬券の時計では **0/7**。
後着地は 3 件（01/02 +65 分、07/01 +227 分、07/02 +3 時 19 分で D1 記録なし）。
着地数が増えても発走前はゼロ。

| 項目            | 結果                                                            |
| --------------- | --------------------------------------------------------------- |
| 朝の着順予測    | Neon **80/80**（940頭）。Mac host one-shot。container ではない  |
| 体重 rescore    | 発走前 **0/7**。後着地 3。死んではいない                        |
