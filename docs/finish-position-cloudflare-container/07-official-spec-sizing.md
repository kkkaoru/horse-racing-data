...
> これを満たせない場合は Cloudflare path の修正を継続する。Mac launchd を production authority に戻さない。
...
5. 問題なければ predict cron (`*/20 1-11` 等) を有効化し Cloudflare-only production を完了。
   ※ Mac launchd は deprecated local/manual tooling のみで、authority ではない。
