...
> **Superseded production note (2026-06-29):** 現在の本番 authority は Cloudflare
> のみ。Mac launchd / local Docker の記述は移行前の設計履歴・比較用であり、
> 本番 scheduler / authority / fallback / ordering dependency ではない。
...
当時の着順予測は **Mac launchd** が JST 03:00 / 09:30 に `finish-position-predict-daily.sh`
を実行するローカル docker パイプラインで稼働していた。2026-06-03 の CF Container 試行は
...
| Mac の役割                     | 旧 authority + fallback          | 現在は deprecated local/manual tooling のみ             |
...
当時の Mac path では `race-prediction-guard` launchd が 20 分ごとに再実行していた。
...
- 当時は Mac の `race-prediction-guard` launchd (20 分, JST 10:00–20:40) を authority としていたが、現在は Cloudflare 側 coordinator が authority。
...
**Mac は production fallback ではない。反復 on-demand ループは CF が完全に肩代わりする。**
Mac launchd は deprecated local/manual tooling として残す場合でも authority ではない。
...
PHASE 3: Historical dual-run comparison (Mac + CF 並列, 本番 Neon)
PHASE 4: Cutover (CF sole authority; Mac は production から外す)
...
それでも失敗なら Cloudflare path の修正を継続し、Mac launchd を production authority に戻さない。
...
- Historical pilot plan: Mac launchd は比較用に一時併走していた。
- Current rule: 本番 Neon への authority は Cloudflare のみ。Mac と CF の dual-authority 運用を本番前提にしない。
...
- Mac launchd を disabled / deprecated に変更
- CF が sole authority に
- `DEPLOY.md` と `01-current-mechanism.md` の authority 記述を更新

**Mac の位置づけ**:

- launchd を残す場合も deprecated local/manual tooling のみ
- 障害時の本番 fallback / ordering dependency として扱わない
- manual kick は smoke/backfill 用に限定し、production scheduler にしない
...
**この phase は急がない**。残す場合も manual diagnostic value のみであり、
production fallback として扱わない。
...
> Historical: 当時は Mac launchd を pilot 完了まで authority とする計画だった。
> Current: Cloudflare-only production に supersede 済み。
...
> Historical: 当時は Mac launchd を cutover 確認まで authority とする計画だった。
> Current: Cloudflare-only production に supersede 済み。
...
| Mac launchd を本番 authority にしない      | 現在は deprecated local/manual tooling のみ                  |
...
2. **pilot が失敗する可能性がある。** doc 03 §1.4 の通り、6/3 の 90s reap は
   ...
   何らかの residual 制約があれば失敗する。その場合は per-venue chunking を試み、
   Cloudflare path を修正する。Mac launchd を production authority に戻さない。
...
4. **race-hours re-predict (rescore mode) は設計済みだが PHASE 3 まで inactive。**
   ...
   Historical: 当時は Mac `race-prediction-guard` launchd が authority。
   Current: Cloudflare-side coordinator が authority。
