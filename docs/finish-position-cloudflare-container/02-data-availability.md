...
Container, and crucially the pipeline is wired to run on one** (the
...
realtime workers over HTTPS — the same network class the legacy local Docker host used. The
...
- **Postgres (Neon)** — DuckDB `postgres` extension `ATTACH`'s the Neon URL and
  ...
  matches the legacy local Docker host access model and is identical on a CF
  Container — the difference is who sets the env var (deprecated local launchd
  vs the Worker).
...
| **legacy launchd-set `RUN_DATE`**                                                                                                | The Worker already computes JST run date (`time.ts`) and passes it.                                                                                                                                                          | None — Worker path already exists.                                                                                                                                                                                                                                                                                                                                                                                  |
