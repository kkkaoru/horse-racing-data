...
The first 454 source-person rows were capped profile lists. They remain valid for race-list display, but the capped rows are not used as percentage denominators. For foreign trainers without an adequate JV population, every published netkeiba result page was subsequently fetched through the final page at a polite interval. Foreign owner profiles were already complete because each had at most 42 published rows. Parsed and unique row counts matched the published counts for every profile. Carrot Farm and the two Japanese trainers were not re-fetched: their complete ten-year JV populations already exist. The expanded source history has 3,759 rows: jockey 200, trainer 3,454, and owner 105.

The A8 snapshot now keeps each statistical population separate. Jockeys, Carrot Farm, Tanaka, and Takei use the preceding ten years across all Japanese JV/NAR venues. Six foreign trainers and two foreign owners use all results published by netkeiba, including its recorded overseas races. `stats_source`, scope, source person ID, population dates, published row count, and completeness are stored per row and exposed to the UI. No JV and netkeiba counts are added or averaged. The UI labels every row and explains the two scopes. Rows below 20 starts remain suppressed, leaving nine jockeys, eight trainers, and three owners.

This work also found that abbreviated JV names are not identities: `オブライ` resolves to both D. O'Brien and A. O'Brien, so the old name-only snapshot incorrectly attributed their combined 197 starts to A. O'Brien. A. O'Brien's actual JV-code population was only nine starts, while his complete netkeiba-published population contains 2,015 rows. The corrected snapshot uses source person IDs and gives him only the latter population. The same audit found 16 duplicated trainer abbreviations and 27 duplicated jockey abbreviations in the masters; domestic code-based aggregation is tracked separately and must not be deployed during an active meeting without a dedicated impact review.

A request-time ten-year/all-venue aggregation is not operationally safe: a production probe of an uncached nine-year equivalent took 135.992 seconds. A8/04 therefore reads the auditable `oversea_person_win_rate_stats` snapshot. Missing snapshots return no overseas fallback rows rather than triggering an expensive aggregation. This is not a local recurring production job. A recurring implementation for future overseas races must be designed on Cloudflare.
