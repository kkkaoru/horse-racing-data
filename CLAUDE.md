...
- `apps/sync-realtime-data/` — **TypeScript (Cloudflare Workers)**. See `apps/sync-realtime-data/CLAUDE.md`. Enforced by `vitest.config.ts`: **Lines/Statements/Functions >= 95, Branches >= 89** (Branches is parser-heavy and tracked separately; raise it back toward 95 by removing dead `??` arms + adding tests, never by lowering the threshold).
